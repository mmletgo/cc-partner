...
///     launcher；确保 `resources/browser-runtime` 与 `resources/web-dist` 可被 Tauri resource glob
///     匹配；最后委托 `tauri_build::build()` 生成 Tauri 所需的编译期上下文。
fn main() {
...
fn ensure_browser_runtime_resource() {
    ...
    let usable = resource_dir_has_entries(&runtime_root);
    if !usable {
        replace_unusable_resource_path(&runtime_root);
        if let Err(error) = fs::create_dir_all(&runtime_root) {
            panic!("创建 browser-runtime 资源目录失败: {error}");
        }
    }

    if resource_dir_has_entries(&runtime_root) {
        return;
    }
...
/// 保证 Tauri `bundle.resources` 的 `resources/web-dist/**/*` 至少能匹配一个文件。
///
/// Business Logic（为什么需要这个函数）:
///     `/mobile` 静态资源目录 `src-tauri/resources/web-dist` 由
///     prepare-web-dist-resource.mjs 在 `tauri build` 时生成且被 gitignore；
///     干净克隆或未构建前端时目录不存在/为空，`tauri_build` 会因 glob 未匹配
///     直接失败，阻断 `cargo check` / `tauri dev` / clippy。
///
/// Code Logic（这个函数做什么）:
///     若路径是损坏/自指 symlink 或普通文件则删除后重建目录；目录为空时写入
///     `.web-dist-unavailable` 占位。已有真实 dist 内容则不动。
fn ensure_web_dist_resource() {
    let web_dist_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("resources")
        .join("web-dist");
    println!("cargo:rerun-if-changed=resources/web-dist");
    ...
    if !resource_dir_has_entries(&web_dist_root) {
        ...
        if let Err(error) = fs::create_dir_all(&web_dist_root) {
            panic!("创建 web-dist 资源目录失败: {error}");
        }
    }
    ...
    let placeholder = web_dist_root.join(".web-dist-unavailable");
    if let Err(error) = fs::write(
        &placeholder,
...
/// 删除无法作为资源根的路径（损坏 symlink 或普通文件）。
///
/// Code Logic（这个函数做什么）:
///     只对 symlink / 普通文件 `remove_file`，避免 `remove_dir_all` 误删 symlink 指向的目标。
fn replace_unusable_resource_path(path: &Path) {
