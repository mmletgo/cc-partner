...
use crate::workbench::tmux_persist::workbench_tmux_persist_conf_path;
pub(crate) use crate::workbench::tmux_persist::{
    tmux_server_persist_commands, tmux_session_persist_commands, tmux_start_server_args,
};
...
/// tmux window user option：绑定工作台 tab id，防止 session 重建后 `@N` 被新窗口占用。
const TMUX_WINDOW_SESSION_ID_OPTION: &str = "@cc-partner-session-id";
...
///
/// Code Logic（这个函数做什么）:
///     生成 `detach-client -s <session>`，一次拆掉该 worktree session 的全部 client。
fn tmux_detach_session_clients_args(session_name: &str) -> Vec<String> {
    vec![
...
/// Business Logic（为什么需要这个函数）:
///     把工作台 tab id 写到 tmux window，restore 才能区分「还是原来的窗」和「同名 session 重建后的新 @0」。
///
/// Code Logic（这个函数做什么）:
///     生成 `set-option -w -t <target> @cc-partner-session-id <session_id>`。
fn tmux_set_window_identity_args(target: &str, session_id: &str) -> Vec<String> {
    vec![
        "set-option".to_string(),
        "-w".to_string(),
        "-t".to_string(),
        target.to_string(),
        TMUX_WINDOW_SESSION_ID_OPTION.to_string(),
        session_id.to_string(),
    ]
}

/// Business Logic（为什么需要这个函数）:
///     restore 需要读出 window 上的 tab id，才能拒绝占用了旧 `@N` 的新窗口。
///
/// Code Logic（这个函数做什么）:
///     生成 `show-options -wqv -t <target> @cc-partner-session-id`。
fn tmux_read_window_identity_args(target: &str) -> Vec<String> {
    vec![
...
/// Business Logic（为什么需要这个函数）:
///     旧窗口没有 tag 时仍应能恢复；新窗口若 tag 对不上，说明 `@N` 已被别人占用。
///
/// Code Logic（这个函数做什么）:
///     trim 后空串视为 legacy 放行；否则必须与 workbench session id 全等。
fn tmux_window_identity_allows_restore(stored_tag: &str, session_id: &str) -> bool {
    let tag = stored_tag.trim();
    tag.is_empty() || tag == session_id
}
...
fn apply_workbench_tmux_status_theme(
    ...
    for args in tmux_session_persist_commands(session_name) {
        let arg_refs = args.iter().map(String::as_str).collect::<Vec<_>>();
        run_tmux_command(tmux, &arg_refs)?;
    }
...
/// Business Logic（为什么需要这个函数）:
///     restore 不能只看 `session:@N` 还在：session 被重建后新窗口会从 `@0` 重编号，
///     旧 tab 会 attach 到别人的窗。
///
/// Code Logic（这个函数做什么）:
///     window 存在且 identity tag 为空（legacy）或等于 workbench session id 才返回 true。
fn tmux_restore_target_usable(
    tmux: &TmuxCommand,
    session_name: &str,
    ...
    match window_id {
        Some(window_id) => {
            ...
            tmux_target_exists(tmux, &target)
                && tmux_window_identity_matches(tmux, &target, workbench_session_id)
        }
        None => tmux_target_exists(tmux, session_name),
    }
}

/// Business Logic（为什么需要这个函数）:
///     读取 window 上的 tab id；命令失败当作未打标的 legacy 窗，避免误杀旧会话恢复。
///
/// Code Logic（这个函数做什么）:
///     show-options 成功则按 tag 匹配；失败视为空 tag。
fn tmux_window_identity_matches(
    tmux: &TmuxCommand,
    target: &str,
    workbench_session_id: &str,
) -> bool {
    let args = tmux_read_window_identity_args(target);
    let arg_refs: Vec<&str> = args.iter().map(String::as_str).collect();
    let tag = run_tmux_command(tmux, &arg_refs).unwrap_or_default();
    tmux_window_identity_allows_restore(&tag, workbench_session_id)
}

/// Business Logic（为什么需要这个函数）:
///     新建和成功 restore 的 window 都要打上 tab id，后续热更新才不会把 `@N` 认错。
///
/// Code Logic（这个函数做什么）:
///     best-effort `set-option -w`；失败只记 debug。
fn ensure_tmux_window_identity(tmux: &TmuxCommand, target: &str, session_id: &str) {
    let args = tmux_set_window_identity_args(target, session_id);
    let arg_refs: Vec<&str> = args.iter().map(String::as_str).collect();
    if let Err(error) = run_tmux_command(tmux, &arg_refs) {
        tracing::debug!("标记 tmux window 身份失败 target={target}: {error}");
    }
}
...
///
/// Code Logic（这个函数做什么）:
///     仅 tmux row 且有 backend_id 时执行 `detach-client -s`；失败忽略。
fn detach_tmux_clients_for_row(row: &WorkbenchSessionRow) {
    if row.backend != TMUX_BACKEND {
        return;
    }
    let Some(session_name) = row.backend_id.as_deref().filter(|name| !name.is_empty()) else {
        return;
    };
    let Some(tmux) = available_tmux_command() else {
        return;
    };
    let args = tmux_detach_session_clients_args(session_name);
    let arg_refs: Vec<&str> = args.iter().map(String::as_str).collect();
    if let Err(error) = run_tmux_command(&tmux, &arg_refs) {
        tracing::debug!("shutdown detach tmux client 失败 session={session_name}: {error}");
    }
}
...
fn create_tmux_window(
    ...
    if output.status.success() {
        ...
        if window_id.is_empty() {
            ...
            let target = tmux_window_target(session_name, &window_id);
            if let Some(session_id) = agent_ctx
                .map(|ctx| ctx.terminal_session_id.as_str())
                .filter(|id| !id.is_empty())
            {
                ensure_tmux_window_identity(tmux, &target, session_id);
            }
...
impl WorkbenchSessionRegistry {
    ...
    pub fn attach_existing_tmux_only(
        ...
        if !tmux_restore_target_usable(
            ...
            row.backend_window_id.as_deref(),
            &row.id,
        ) {
            return Err(AppError::unavailable("tmux_target_missing".to_string()));
        }
        ...
        if let (Some(tmux), Ok(target)) = (available_tmux_command(), tmux_target_for_row(&restored))
        {
            ensure_tmux_window_identity(&tmux, &target, &restored.id);
        }
    ...
    ///     必须先 `detach-client` 再 SIGHUP PTY，避免 hangup 拆掉 pane。
    ///     R42 M4：close kill 失败后的 generation-scoped handle 也必须再试 kill，
    ///     否则进程退出后内存句柄丢失、raw PTY 可能成为不可恢复孤儿。
    ///
    /// Code Logic（这个函数做什么）:
    ///     遍历 registry：tmux row 先 detach-client，再 kill 仍运行的 PTY child，内存状态标 disconnected；
    ...
    pub fn shutdown_all(&self) -> usize {
        ...
        for handle in handles {
            ...
            if was_running {
                detach_tmux_clients_for_row(&handle.row);
            }
            match &mut handle.process {
                SessionProcess::Pty { child, .. } => {
                    if was_running {
...
// ---------------------------------------------------------------------------
// 单测：`sessions_test.rs`（文件名含 test，module-boundary 门禁排除）
// ---------------------------------------------------------------------------
