...
///
/// Code Logic（这个结构做什么）:
///     纯数据载体；`role` 仅 `"wifi"` | `"wired"`，未知为 None；
///     由 mobile 模块映射为 `MobileAccessRole` 后进入 access-info DTO。
...
///     由 `mobile::mobile_access_info_from_state` 消费并映射为 access-info entries。
...
mod tests {
    ...
    fn service_info_advertises_proto_and_caps_hints() {
        ...
        // encode_mdns_capabilities 按字典序排序去重，受 MAX_CAPS_TXT_BYTES 截断；与 advertise 同源。
        let expected =
