//! Gemini CLI visible terminal adapter。
...
impl GeminiCliAdapter {
    fn probe_version() -> Result<(Option<String>, Option<String>), AppError> {
        let mut child = Command::new("gemini")
            ...
            .spawn()
            .map_err(|_| AppError::generic("gemini executable unavailable"))?;
            ...
            match child.try_wait() {
                Ok(Some(status)) => {
                    ...
                    if status.success() {
                        ...
                        return Ok((Some("gemini".into()), version));
                    }
                    return Ok((None, None));
                }
                ...
                Ok(None) => {
                    let _ = child.kill();
                    let _ = child.wait();
                    return Err(AppError::timeout("gemini --version 超时"));
                }
                Err(err) => return Err(AppError::generic(format!("gemini probe 失败: {err}"))),
            }
        }
    }
}
