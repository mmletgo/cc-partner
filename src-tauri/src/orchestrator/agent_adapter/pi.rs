//! Pi Coding Agent visible terminal adapter。
...
impl PiAdapter {
    fn probe_version() -> Result<(Option<String>, Option<String>), AppError> {
        let mut child = Command::new("pi")
            ...
            .spawn()
            .map_err(|_| AppError::generic("pi executable unavailable"))?;
            ...
            match child.try_wait() {
                Ok(Some(status)) => {
                    ...
                    if status.success() {
                        ...
                        return Ok((Some("pi".into()), version));
                    }
                    return Ok((None, None));
                }
                ...
                Ok(None) => {
                    let _ = child.kill();
                    let _ = child.wait();
                    return Err(AppError::timeout("pi --version 超时"));
                }
                Err(err) => return Err(AppError::generic(format!("pi probe 失败: {err}"))),
            }
        }
    }
}
    ...
    fn build_resume_plan(&self, request: &AgentLaunchRequest) -> Result<AgentLaunchPlan, AppError> {
            ...
            .filter(|v| !v.is_empty())
        {
            return Ok(AgentLaunchPlan {
                executable: "pi".into(),
                args: vec!["--session".into(), native.to_string()],
                stdin: None,
                env: vec![],
                completion: AgentCompletionContract::Manual,
            });
