//! Grok Build visible terminal adapter。
...
impl GrokBuildAdapter {
    fn probe_version() -> Result<(Option<String>, Option<String>), AppError> {
        let mut child = Command::new("grok")
            ...
            .spawn()
            .map_err(|_| AppError::generic("grok executable unavailable"))?;
            ...
            match child.try_wait() {
                Ok(Some(status)) => {
                    ...
                    if status.success() {
                        ...
                        return Ok((Some("grok".into()), version));
                    }
                    return Ok((None, None));
                }
                ...
                Ok(None) => {
                    let _ = child.kill();
                    let _ = child.wait();
                    return Err(AppError::timeout("grok --version 超时"));
                }
                Err(err) => return Err(AppError::generic(format!("grok probe 失败: {err}"))),
            }
        }
    }
}
