...
- **核心模型**：把一个 GitHub 私有仓库当作"中心化对端"。**本地 SQLite + 向量时钟是权威源**，git 只承担传输与历史承载——不参与合并，只保证最终文件一致。一次同步 = detect_git → ensure_repo → 定分支 → import(merge 进本地) → export(写回工作区) → commit → push 循环。冲突解决完全复用既有 `merge_prompt`/`merge_claude_md`/`merge_cc_history`/`merge_ssh_target`（向量时钟 + LWW + device_id tie-break），与局域网同步语义一致。同步范围与局域网一致：prompts + CLAUDE.md + CC 历史 + SSH 目标（含软删除传播）。
...
- **工作区文件结构**（`cloud_sync/snapshot.rs`）：
  - `prompts/<id>.json` → PromptRow；`claude_md/claude_md.json` → ClaudeMdRow（单例固定名）；`claude_history/<id>.json` → ClaudeHistoryRow；`ssh_targets/<host>.json` → SshTargetRow。
