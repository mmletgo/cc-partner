...
- **终端事件**：后端 emit `workbench:terminal-output`（`sessionId/chunk/seq/ts`）和 `workbench:terminal-status`（`sessionId/status/exitCode/ts`）；PTY reader 必须跨 read chunk 做流式 UTF-8 解码，避免中文/符号被拆包后在前端显示为 `�`；前端按 sessionId 维护 buffer。普通 Vite 浏览器无 Tauri event internals 时前端必须跳过 listen，避免调试白屏。`workbench/remote_events.rs` 会把本机 terminal/merge 事件发布到 AppState broadcast channel，`GET /api/workbench/events` 以 NDJSON 输出；本机访问 remote 项目 session list/create 前必须先确保事件桥连接并写入 project 映射，避免恢复或创建过程中丢失早期 output/status 事件。事件桥读取 NDJSON 时用 byte buffer 按完整换行解析，避免中文/emoji 跨网络 chunk 乱码；terminal sessionId 映射为 `remote:<deviceId>:<innerId>`，merge progress 的 worktreeId 也映射为 remote entity，projectId 则用桥接上下文里的 `innerProjectId -> 本机 remote shortcut projectId` 映射，确保前端按当前 shortcut project 过滤事件。
- **会话 replay buffer**：`WorkbenchSessionRegistry` 为每个运行期 session 维护最近输出 ring buffer，PTY reader 在发送 `workbench:terminal-output` 前用同一 `chunk/seq` 写入 replay；截断必须按 Unicode char 边界保留尾部，避免中文/emoji 被切坏。移动端通过 `POST /api/workbench/sessions/replay` body `{sessionId}` 拉取 `WorkbenchSessionReplayDto{sessionId,buffer,truncated,lastSeq}`，route 必须先复用 local session guard 并确认 session 仍在 registry；关闭 session 时同步清理 replay buffer，退出 `shutdown_all` 只断开运行态不删除元数据。
...
- **/mobile 静态 SPA**：`http_server.rs` 在所有 `/api/*` 路由之后、body limit layer 之前挂 axum fallback service，只接管 `/mobile` 与 `/mobile/...`；fallback service 捕获 `AppState`，把 `/mobile`/`/mobile/` 规范化为 asset key `mobile.html`，把 `/mobile/<asset>` 规范化为不含 `/mobile` 前缀的 frontendDist 相对 key（如 `assets/index.js`），优先用 `state.app_handle.asset_resolver()` 读取 Tauri 嵌入资源，生产打包后不依赖源码 `../web/dist`；生产态有嵌入资源时必须先检查 exact asset key，避免 Tauri resolver 缺省回退到桌面 `index.html`，资源缺失时由本模块 fallback 到 `mobile.html`，shell 缺失则 404。filesystem 读取 `../web/dist/<key>` 仅作为 dev/test 兜底，不能作为唯一生产路径。静态资源路径必须用 `Component` 校验拒绝空路径、绝对路径、`..` 和其它非 normal component；响应优先使用 Tauri asset MIME，缺失时用本地 `mobile_content_type` 映射。未知非 `/mobile` 路径保持 404，不能捕获 `/api/workbench/events` 或 `/api/mobile/access-info`。验证命令：`cargo test net::http_server::tests --lib`。
...
- **Prompt 优化命令**：`optimize_prompt(prompt, workingDirectory?, targetLanguage?)` 复用 `AppConfig.github_trending` 中的 `claude_cli_path` 与 `claude_model`，不新增配置入口；普通 Prompt 优化页不传目录和 targetLanguage 并保持 pure/bare 双语模式，Workbench 若需要完整单语结果可传当前项目根目录和设置页语种 `zh|en`。`stream_optimize_prompt_to_workbench_session(prompt, workingDirectory?, targetLanguage, sessionId)` 是 Workbench 快捷键小组件专用命令，使用 stream-json 纯文本输出，把每个 assistant 文本增量写入指定 running terminal session；当 sessionId 为 `remote:<deviceId>:<innerId>` 时，本机只校验/剥离 remote 前缀并把请求代理到远端 `/api/workbench/prompt-optimizer/stream-to-session`，由远端用远端 worktree 路径运行 Claude CLI 并写入远端 session。空输入和超过 20,000 字符的输入直接返回业务错误，非空本机 workingDirectory 必须存在且是目录，结构化命令 targetLanguage 接受 `zh` / `en` / 空，流式命令必须传 `zh` 或 `en`；CLI 调用超时 180 秒。
