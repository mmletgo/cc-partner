...
- **/mobile 静态 SPA**：`http_server.rs` 在所有 `/api/*` 路由之后、body limit layer 之前挂 axum fallback service，只接管 `/mobile` 与 `/mobile/...`；`/mobile`/`/mobile/` 返回 `web/dist/mobile.html`，`/mobile/<asset>` 优先读取 `web/dist/<asset>`，资源缺失时 fallback 到 `mobile.html`，shell 缺失则 404。静态资源路径必须用 `Component` 校验拒绝绝对路径、`..` 等目录穿越；未知非 `/mobile` 路径保持 404，不能捕获 `/api/workbench/events` 或 `/api/mobile/access-info`。验证命令：`cargo test net::http_server::tests --lib`。
