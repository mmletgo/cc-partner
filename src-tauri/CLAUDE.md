...
- **文件内容浏览/编辑**：`workbench/file_preview.rs` 按扩展名和轻量内容特征检测 Markdown、HTML、JSON、TOML、YAML、CSV、SQLite、图片、代码和普通文本，并返回前端能力矩阵（预览/编辑/格式化/默认模式）；`workbench/file_content.rs` 负责文本读取、baseHash 乐观锁原子保存、JSON/TOML/YAML 语义校验与格式化；`workbench/sqlite_preview.rs` 只读枚举 SQLite 用户表并按表名安全 quoting 查询前 N 行，不执行用户 SQL。`commands/workbench.rs` 暴露 `open_workbench_file`、`save_workbench_text_file`、`format_workbench_structured_content`、`preview_workbench_sqlite` 四个文件内容命令：打开文件返回 metadata/capabilities 与对应 preview/text payload，保存时以后端真实文件名重新检测类型，HTML 按文本保存，JSON/TOML/YAML 做语义校验，格式化只返回文本不写盘，SQLite 预览只允许当前 active worktree 根内文件。
