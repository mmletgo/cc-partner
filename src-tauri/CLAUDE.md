...
- **移动端项目列表 HTTP 路由**：`net/routes/workbench.rs` 暴露 `GET /api/workbench/projects/list`，语义对齐 `list_workbench_projects` 命令：从 `workbench_project_repo.list()` 读取最近项目并用 `WorkbenchProjectRow::to_dto` 返回 camelCase DTO，供 `/mobile` 普通浏览器 shell 获取项目入口。
