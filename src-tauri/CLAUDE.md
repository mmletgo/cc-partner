...
├── workbench/         — 本机项目工作台：项目记录 + Git worktree + tmux 依赖管理 + 可恢复 PTY/tmux 终端会话 + 安全文件树 + 文件内容/预览 [已实现]
...
- **功能定位**：Workbench 是本机项目运行态工作台，前端入口 `/workbench`。一期只覆盖本机或已挂载局域网目录；远端 cc-partner 项目浏览、远端 PTY 和远端文件预览后续单独扩展。
