...
- **提醒触发（`health/mod.rs::handle_sample`）**：每分钟 → 写活动记录 →（enabled && !paused）推进状态机 → `should_remind` 且未贪睡且不在 DND 时段且 `notify_enabled` → emit `health:reminder` 事件（载荷 `{workWindowSeconds}`），前端 `HealthReminderListener` 监听后弹 i18n 系统通知；若 `reminder_fullscreen` 开启则 emit 后调 `open_health_overlay(app, "reminder")` 每屏弹透明置顶遮罩窗口（Plan 2）。**系统通知统一为前端 i18n 出口**：后端不再用 `tauri-plugin-notification` 发系统通知（早期 `send_system_notification` 硬编码中英文文案 + 前端监听又发一条 = 双通知，已删除后端发送 + 函数定义 + `NotificationExt` import，后端只 emit 事件）。喝水提醒同理：超间隔且无 pending 且非 DND → 置 pending + emit `health:water`（前端弹 i18n 系统通知）+ 若 `reminder_fullscreen` 则 `open_health_overlay(app, "water")` 每屏弹遮罩（与久坐同 gate 同模式）。跨 await 不持 RwLockReadGuard：开头 `state.config.read().unwrap().health.clone()` 先 clone 配置副本释放锁。DND 判定（`health/reminder.rs::is_in_dnd`）支持跨午夜（如 22:00-07:00），按**系统本地时区**取时分判定（`chrono::Local::from_timestamp`，不引 chrono-tz），与用户本地作息一致。
- **全屏遮罩提醒（Plan 2，`health/mod.rs::open_health_overlay`/`close_health_overlay`）**：开启 `reminder_fullscreen` 后，久坐/喝水提醒 emit 后每屏建一个透明置顶遮罩窗口（复用截图透明窗口构建模式：`WebviewWindowBuilder` decorations(false)/transparent(true)/always_on_top(true)/focused(true)/skip_taskbar(true)/resizable(false)），label = `health-overlay-{i}`，url = `/health-overlay?display={i}&type={overlay_type}`（`overlay_type` = `"reminder"` 或 `"water"`，前端遮罩页据此渲染对应文案与按钮），几何直接用 xcap `monitor.x()/y()/width()/height()`（逻辑点，不除 scale，与截图 overlay 一致）。窗口需 `app.macOSPrivateApi: true`（已开）。`close_health_overlay` 遍历 `webview_windows()` 关闭 `health-overlay-` 前缀全部窗口（供前端遮罩页推迟/跳过按钮经 `close_health_overlay` 命令调用）。capabilities `default.json` windows 数组含 `health-overlay-*` 通配（遮罩页 invoke 才被放行）。
