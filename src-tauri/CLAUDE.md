...
- **Git worktree 管理**：`workbench_worktrees` 表持久化项目下工作区，字段 `id/project_id/name/branch/base_branch/path/is_main/created_at/updated_at`；主 worktree 使用确定性 id `{project_id}:main`，路径等于项目根目录，`list_workbench_worktrees(projectId)` 会确保主记录存在，并调用 `git worktree list --porcelain` 把磁盘上已有但 SQLite 未记录的 Git worktree 按 path 稳定导入，再注入实时 `git status --porcelain --branch` 摘要；导入已有 worktree 时优先复用相同 path 的旧 row，新的外部 worktree 使用 project_id + path 的确定性 SHA256 id，避免每次载入重复显示。status 按 upstream/origin 规则派生 `canPush`，本地未发布且没有 origin/upstream 的项目应让前端 Push 按钮禁用，只有 `*-upstream` 这类源码上游 remote 也不能启用。`create_workbench_worktree(projectId, branchName, baseBranch?)` 在应用数据目录 `worktrees/<project_id>/<branch_slug>` 下执行 `git worktree add -b`；`commit_workbench_worktree(worktreeId,message?)` 在 message 为空时执行 `git add -A`、读取 staged diff/stat，并在 worktree cwd 下用 Claude Code 项目上下文模式生成 commit message 后 `git commit -m`，message 非空时保留手写提交兼容路径；无可提交改动时该命令返回最新 worktree DTO 而不是错误，用于刷新前端 stale 状态；`push_workbench_worktree` 优先复用当前分支 upstream 执行 `git push`，没有 upstream 时只选择 `origin` 执行 `git push -u origin <branch>`，没有 origin/upstream 时返回可操作提示；`merge_workbench_worktree` 只允许非主 worktree 且源/主工作区均 clean、无冲突时在主工作区执行 `git merge --no-ff <branch>`；`remove_workbench_worktree` 禁止删除主工作区，且要求先关闭该 worktree 下的 terminal window，再执行 `git worktree remove` 并删元数据；`list_workbench_git_commits(projectId, worktreeId?, limit?)` 在 active worktree cwd 下用 `git log --all --topo-order --decorate=full` 读取提交 DAG、父提交和 refs，供右侧栏 Git 历史 tab 绘制提交树并区分本地/远端/tag。第一期不做 diff viewer、交互式冲突解决或 PR 创建。
...
- **验证命令**：相关 Rust 验证优先跑 `cd src-tauri && cargo test workbench::sessions --lib && cargo test workbench::git --lib && cargo test storage::workbench_worktree_repo --lib && cargo test storage::workbench_session_repo --lib && cargo check`；前端联动验证跑 `cd web && npm run build` 与 `npx tsx src/pages/Workbench/workbenchWorktrees.test.ts`，必要时再用浏览器检查 `/workbench`。
