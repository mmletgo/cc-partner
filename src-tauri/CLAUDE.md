...
├── workbench/         — 本机项目工作台：项目记录 + Git worktree + tmux 依赖管理 + 可恢复 PTY/tmux 终端会话 + 安全文件树 + 文件内容/预览 + 远端目录选择 helper 与 HTTP 路由 [已实现]
...
- **功能定位**：Workbench 是本机项目运行态工作台，前端入口 `/workbench`。本机项目继续直接操作本机文件/Git/worktree/session；远端 cc-partner 项目通过本机 remote shortcut + 对端 P2P HTTP 路由代理 worktree 列表/创建、Git 提交列表、文件树/文件内容/基础文件变更、terminal session 控制与事件流。Prompt optimizer routing 仍只走本机命令，未做远端代理。
...
- **远端项目快捷方式与网关命令**：`workbench/remote_client.rs` 是本机调用远端 Workbench HTTP 路由的 reqwest client，统一处理 roots/list/info/open-project，以及 worktrees/list/create、git/commits、files/list-dir/info/open/save-text/create-file/create-dir/rename/delete、sessions/list/create/write/resize/focus/focused/split-pane/close-pane/close/rename 的中文错误映射；共享请求 DTO 放在 `workbench/remote_protocol.rs`。`commands/workbench.rs` 先从 `state.devices` 解析 `device.base_url()`，打开项目后只在本机 `workbench_projects` 保存 `kind="remote"` 的快捷方式，id 固定为 `remote_project_id(device_id, remote.path)`，不新增数据库 schema。后续 worktree/Git/files/session 命令若遇到 `project.kind=="remote"`，会用远端 `/api/workbench/projects/open` 按 shortcut path 恢复/确保远端 local project row，再把返回的远端 worktree/session id 映射成 `remote:<deviceId>:<innerId>`；发回远端时必须先用 `parse_remote_entity_id` 剥离前缀并校验设备一致。Workbench 远端 HTTP 路由面向可信局域网/自有设备，不加 auth/token/permission gate；`net/routes/workbench.rs` 只做协议校验，worktree/Git/files/session 路由必须确认收到的是对端 local project/session row，避免 remote shortcut 递归代理。
