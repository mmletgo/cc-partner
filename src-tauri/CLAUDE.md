...
- **功能定位**：Workbench 是本机项目运行态工作台，前端入口 `/workbench`。当前本机 UI/API 仍以本机或已挂载局域网目录为主；远端 cc-partner 项目能力分阶段扩展，已暴露远端目录选择与远端打开项目 HTTP 路由，远端 PTY、文件和 Git 代理后续单独扩展。
...
- **远端项目快捷方式命令**：`workbench/remote_client.rs` 是本机调用远端 Workbench HTTP 路由的 reqwest client，统一处理 roots/list/info/open-project 和中文错误映射；`commands/workbench.rs` 暴露 `list_workbench_remote_roots`、`list_workbench_remote_dir`、`get_workbench_remote_path_info`、`open_workbench_remote_project`，先从 `state.devices` 解析 `device.base_url()`，打开项目后只在本机 `workbench_projects` 保存 `kind="remote"` 的快捷方式，id 固定为 `remote_project_id(device_id, remote.path)`，不新增数据库 schema。
