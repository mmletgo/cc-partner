...
- **退出清理**：`RunEvent::Exit` 必须调用 `state.workbench_sessions.shutdown_all()`，kill 当前运行期 PTY attach 并把内存状态标记为 disconnected；不得删除 `workbench_sessions` 元数据或销毁 tmux session，否则重启后无法恢复上下文。
