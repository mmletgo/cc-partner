...
- **Provider 管理** — `/provider-manager` 查询并切换 cc-switch 已配置的各 agent provider（读 cc-switch SQLite + 委托 CLI 写盘，不编辑详情、不带 API key）；支持本机与局域网远端设备（含跳板影子设备）目标切换与 cc-switch CLI 远程安装
...
| Dialog | open, titleId, onClose, closeOnEscape?, closeOnBackdrop?, initialFocusRef?, className?, backdropVariant? | portal 模态；surface `role=dialog aria-modal` + **默认 `padding: var(--space-5)`**；`backdropVariant` 默认 `frost`（半透明+blur），`scrim` 仅半透明无模糊；共享 `useModalLayer`（focus trap / Escape / 背景 inert 引用计数 / body scroll lock / 关闭恢复触发焦点）；禁止业务页自建 focus trap。嵌套 Card 或自管分区 padding 时，`className` 必须显式 `padding: 0` 覆盖，避免双边距 |
...
| WorkbenchWorkspaceSwitch | value, onChange, options, ariaLabel | Workbench worktree 行最右三段 radiogroup 切换控件（终端 / 网页浏览 / 文件浏览），绑定 workspaceView；terminal 全屏时仍渲染，browser 在无 project/worktree 时 disabled，files 默认可点（无打开文件时进入空白页并切右侧栏到项目文件夹），键盘 ArrowLeft/Right/Home/End 循环切换；组件自身禁止 `container-type: inline-size`（会塌缩自身宽度导致溢出裁切），窄宽收起文字由 `.worktreeBar` 声明的命名容器 `workbench-workspace-switch` 驱动（行宽 ≤640px 只留图标） |
...
| Health 能力 token | `server_protocol_info()` 字典序全集；本轮新增含 `agent-hub.portable-user.v1` · `agent-hub.user-instructions.v1` · `agent-hub.user-mirror.v1` · `net.relay.v1` · `orchestrator.complete-agent-run.v1` · `orchestrator.move-workflow-state.v1` · `workbench.banner.v1` · `workbench.dependency-install.v1` · `workbench.fs.create-dir.v1` · `workbench.fresh-restart.v1` · `workbench.hook-repair.v1` · `workbench.project-notes.v1` · `workbench.worktree-pull.v1`（权威清单见 `docs/p2p-protocol.md`） |
