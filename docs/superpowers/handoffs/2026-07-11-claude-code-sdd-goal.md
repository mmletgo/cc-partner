...
所有 task、Plan-level 和 whole-program reviewer 必须由 Claude Code 通过已安装的 [**codex-plugin-cc**](https://github.com/openai/codex-plugin-cc) 调用 Codex。只能使用该插件暴露的 `/codex:*` slash commands；**禁止直接执行 `codex` CLI、禁止直接调用插件内部 script/companion runtime，也禁止用 Bash 绕过插件**。Claude 主控、implementer 或其他 Claude subagent 不得充当 reviewer。预检运行 `/codex:setup`；插件报告 Codex CLI/认证不可用时即报告 blocker，禁止自行改用 CLI、降级或跳审。自定义审查统一使用：
