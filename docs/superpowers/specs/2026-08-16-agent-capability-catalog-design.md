...
- 不为 Grok / OpenCode / Gemini / Cursor / Pi 开放 portable-store 原生写入（无 L3 则 inventory/preview only）。「确认当前版本」不是原生写入：无 L3 仍可 apply，只对齐 Hub 账本。「恢复为仓库资产」是 native 路径上的布局修复：无 L3 仍可 apply，把逃逸目标复制进 store 并挂正规软链，不删源树。
...
- 不把逃逸软链「恢复为仓库资产」绑到 CLI allowlist，或做成 `migrateToStore` 那种 native 真树 rename，或删除 `~/.agents`。
...
| 恢复逃逸软链为仓库资产 | 可 apply；复制目标进 store，native 挂正规软链，不删源 | 同左 |
