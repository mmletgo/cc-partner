...
- 在项目文件夹维度管理多个 Claude Code 终端，并直接操作项目文件树
...
**描述**：以项目文件夹为中心管理本机 Claude Code 交互式终端和项目文件夹。第一期支持本机目录及已挂载局域网目录；远端 cc-partner 设备上的原生远端项目、远端 PTY 和文件内容预览后续扩展。

**功能点**：
- 工作台布局：项目文件夹列表紧跟全局左侧栏“设置”菜单项下方，作为进入工作台的入口，不再设置独立“工作台”主导航项；主区域依次展示工作台标题、Claude Code sessions 标识、当前项目上下文、会话 tabs 和终端面板；右侧检查器仅承载当前会话状态与项目文件夹交互，窄宽下排到首屏终端之后
...
- 多 Claude Code 终端：在当前项目目录中通过 PTY 启动交互式 `claude`，支持多个会话 tab
- 终端输出：PTY 输出必须保留中文与符号等 UTF-8 文本完整性；创建终端时应使用当前终端可见 viewport 的真实 cols/rows 作为 PTY 初始尺寸，终端运行时 resize 也必须按同一可见 viewport 计算，避免 Claude Code 状态栏和命令行内容出现替换字符、首屏错位或内容超出工作台可视宽度
- 终端分屏：中央终端区支持单窗、双列、四宫格，分别展示焦点会话以及最近会话，右侧状态始终跟随焦点会话
- 会话状态：右侧只展示当前会话状态，包含设备、项目名、项目路径、会话名、命令、状态、运行时长、尺寸、开始时间和退出码
- 会话操作：支持重命名、关闭；关闭/应用退出清理时如果底层进程已自然退出或被系统回收，应视为清理成功，不向用户展示 No such process 类 IO 错误；应用退出时必须清理仍运行的工作台 PTY 子进程，避免后台残留 Claude Code
...
- 存储：SQLite + sqlx
- 抓屏/剪贴板：xcap + arboard
- 终端：portable-pty（工作台交互式 Claude Code PTY）
