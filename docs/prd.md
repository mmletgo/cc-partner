...
- 依赖环境页签展示 Workbench tmux dependency manager 状态，支持检测、查看后端/版本/路径、查看安装命令预览、触发安装、取消安装和重新检测；同时展示局域网互联访问项目所需的实际 P2P HTTP TCP 端口、mDNS UDP 5353、局域网 IP、当前系统平台和对应手动放行方法。应用不自动修改防火墙，也不声称可无权限可靠判断真实放行状态；移动端 Workbench 的局域网访问链接与二维码由全局侧栏左下角手机按钮弹层展示
