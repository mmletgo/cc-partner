...
- 移动端 Workbench：桌面端 HTTP server 默认提供 `/mobile` 普通浏览器入口；移动端先加载最近项目列表，只有本机项目可选择并通过 HTTP 加载该项目 worktree 与 terminal window 列表，远端项目快捷方式保留展示但禁用选择，避免进入未实现的“手机 → 本机 → 另一远端设备”二级代理路径；默认 active worktree 优先主工作区、默认 active session 优先匹配当前 worktree 的会话，并在移动端 shell 状态栏展示当前项目、worktree 和 session；移动端只做展示和 HTTP 访问层，不新增第二套 tmux/session/window/pane 模型
