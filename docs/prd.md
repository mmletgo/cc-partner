...
- 终端输出：PTY 输出必须保留中文与符号等 UTF-8 文本完整性；创建/重启终端时应使用当前终端可见 viewport 的真实 cols/rows 作为 PTY 初始尺寸，终端运行时 resize 也必须按同一可见 viewport 计算，避免 Claude Code 状态栏和命令行内容出现替换字符、首屏错位或内容超出工作台可视宽度
...
- 会话操作：支持重命名、重启、停止、关闭；应用退出时必须清理仍运行的工作台 PTY 子进程，避免后台残留 Claude Code
