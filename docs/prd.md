...
- tmux 冷恢复历史：应用重启后恢复已有 tmux window 时，后端必须在重新 attach 前用精确 window target 捕获 tmux 历史区（不含当前可见屏幕并保留文本属性），将结果按 xterm 原始换行约定写入当前 generation 的有界 replay buffer，再接续 attach 对当前屏幕的重绘和实时输出；不得只依赖新 PTY 收到的当前视口重绘，否则会丢失重启前历史并让最后一屏重复占满 scrollback。前端首次挂载时应以 session 持久化 cols/rows 为已上报基线，FitAddon 结果相同则不得再次 force resize；仅真实尺寸变化、可见性恢复或用户点击“适应尺寸”才允许强制 redraw。捕获失败时允许降级为仅恢复当前视口，但不能阻断 session attach。
