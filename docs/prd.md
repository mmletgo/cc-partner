...
- 浏览器自动验证（A5）：在 owning device 按需启动 ephemeral managed Chromium（Chrome for Testing headless-shell 固定版本），仅绑定 live preview registry 的 `previewId`（不接受任意 target URL/CDP）；默认一键 smoke 产出 accessibility snapshot、console 摘要与 screenshot evidence；click/fill 只接受 generation-bound opaque nodeRef；fill value 永不写入结果/日志；engine 空闲退出并删除临时 profile；remote/mobile 只代理到 owner（capability `workbench.browser-verification.v1`）；不导入 cookie/profile、不提供任意 JS eval；验证不可用时现有 iframe 预览仍可用
