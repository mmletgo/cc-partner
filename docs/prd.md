...
- 终端分屏：中央终端区每次 attach 当前 window；左右/上下分屏调用 tmux `split-window -c <项目根目录>` 创建真实 pane，新 pane 必须从项目根目录启动而不是继承当前 pane 的 cwd；关闭 pane 调用 tmux `kill-pane`，最后一个 pane 必须通过关闭 window 处理
