...
- Browser preview：Workbench 应提供当前项目/worktree 的浏览器预览工作区，用于访问该项目所在设备上的本机 dev server；后端只允许代理 loopback `http(s)` 且显式端口目标，候选来源按上次使用、终端输出、项目配置和端口探测排序，避免成为开放代理。移动端 Workbench 复用同一预览能力，但远端项目必须在 owning device 上发现和访问目标
