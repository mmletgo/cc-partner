...
- 工作台顶栏标语：桌面端已选中项目后，标题区与「项目自动化」按钮之前提供设备级全局可编辑标语（仍非 per-project；2026-08 起「现场快照」二级入口不再在标题行展示，标语位置相应向左靠拢）；支持轻量 Markdown（粗体/斜体/删除线/行内代码/http(s) 链接）与 emoji；单击预览进入编辑，失焦或 ⌘/Ctrl+Enter 保存，Esc 取消未提交草稿；正文落在 owning device SQLite 单行表（上限仍 280 UTF-16），不进 `config.json`、workspace layout、备份可恢复领域或 Git。选中本机项目读写本机标语；选中远端项目经 `POST /api/workbench/banner/{get,save}`（capability `workbench.banner.v1`）读写对端标语。本机首次读空可一次性从 legacy `localStorage`（`cp-workbench-banner`）灌入后停用该 key；缺 capability 为 unsupported，禁止静默再写 localStorage。远端离线只读上次成功值或空并禁用编辑。按区域尺寸二分最大字号使全文可见并尽量撑满，不拉字距、不压扁字形；零项目/继续工作页/终端全屏/移动端 `/mobile` 不展示。不新增第 8 个 Workbench controller。
