...
- 同一 Git 仓库合并：侧栏按规范化 Git remote（SSH/HTTPS 相同即同一仓库，不限 GitHub）把本机与各设备上的 clone 合成一项；无 remote 的目录不互并。点刷新扫描 origin 并写回 `gitRemoteFingerprint`（离线/读失败保留上次结果）。列表项展示上次打开的路径并提示其他设备；点该项打开上次使用的工作区（设备筛选开启时取该设备上上次使用的成员）。设备/路径切换在工作区标题下拉里完成，不在列表拆成多条。设备筛选只收窄「哪些仓库出现」。删除多项时弹出勾选。底层仍是一行设备×路径，终端/占用/新窗口仍按原 projectId。
