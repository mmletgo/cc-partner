...
- macOS 权限引导：首次启动引导授权 Accessibility / 屏幕录制 / 输入监控（required）；通知为第 4 项且不阻塞继续使用；首轮失败可「重新检查」
...
**功能点**：
- 工作现场安全恢复：桌面端自动保存最后工作现场结构 metadata（project/worktree/session/view/inspector/browser target；slot `desktop:auto`，`schemaVersion=1`，稳定 selection 变化 500ms debounce 合并；revision CAS）。layout **不得**包含 terminal 字节、Prompt/回复、文件正文、env、token、命令、provider 配置或 preview ID。打开 Workbench 时先 side-effect-free preflight，再应用可安全恢复项；tmux target 存在才允许幂等 safe attach，禁止 `tmux new-session/new-window`、raw PTY fallback、terminal write、创建 worktree/shell、Claude/Codex resume。完全成功静默；partial 仅一条可关闭 inline notice。命名 snapshot 仅结构 metadata，非 Command Recipe。remote layout 留在控制设备；owner 只接收 inner ID 做 preflight/attach，capability `workbench.workspace-safe-restore.v1`；Mobile v1 不自动应用 Desktop layout。不新增第 8 个 Workbench controller。
...
- Human Review / Rework 任务详情提供 **Changes（有界 review diff）** 面板：只读展示 owning device 基于 task/worktree 采集的 staged/unstaged/untracked 改动（含 unborn）；展示上限 ≤200 文件、总 patch ≤2MiB、单文件 patch ≤256KiB；binary 仅元数据无 patch 正文；path 必须为 repo-relative（拒绝绝对路径与 `..`）。每份快照携带 `reviewDigest`（SHA-256：base/head 身份 + 按 path 排序的文件元数据 + 完整新内容 hash），**不得**对截断展示 patch 做哈希。桌面/远端/移动共用同一 remote-aware helper；能力 token `orchestrator.review-diff.v1`
- **Deliver 拒绝未审漂移**：对 diff-capable owner，Deliver 请求必须携带非空 `expectedReviewDigest`；owner 在进入交付 pipeline **之前**立即 recollect 并比对 digest；不匹配 → 稳定 conflict `review_diff_changed`，任务保持 Human Review，不得进入 Delivering 或写 delivery evidence；缺/空白 digest → validation。legacy 对端无 `orchestrator.review-diff.v1` 时沿用旧 body（仅 `taskId`）
...
- **运营系统通知（非 Inbox）**：偏好可控，默认 Human Review / Blocked / remote outbox failed 开启、Task Done 关闭；仅消费 owner 事件 `{kind,opaqueSourceId,stateVersion,occurredAt}`（可选 relay 信封 ownerInstanceId/sequence）；title/body 只用通用 i18n，**禁止**任务/项目/goal/diff/evidence 明文，**禁止** actionType/onAction/extra 业务回调；control snapshot `POST /api/backend/control/operational-notifications/snapshot` 为 loopback+token only，**不是** LAN capability。通知与 Attention 只导航，都不执行 Deliver/Rework/Retry 等业务动作
...
- 条目**只导航**（导航-only）：任务/Evidence 进 Orchestrator 自动化控制台，failed outbox 进自动化控制台 outbox 区，tmux 依赖进 Settings `dependencies`；列表内与 deep link 目标落地均**不**执行 Deliver、Request Rework、Retry、Discard 或依赖安装；系统运营通知同样无业务 action 回调，点击至多导航到既有权威界面
