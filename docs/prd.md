...
- Prompt 优化：用户输入原始编程任务 Prompt 后，调用本机 Claude Code CLI 生成优化版 Prompt（结果语言跟随当前界面语种；Workbench 快捷键走流式写入当前终端）
- Prompt 优化结果只用于当前页面展示和复制，不保存历史、不入库、不跨设备同步、不做缓存
- Prompt 优化输出必须以需求方视角写成可直接粘贴给 Claude Code 的委托式 Prompt，不得生成“请确认/是否需要/请指定”等继续询问用户的澄清句；原始信息不足时只能写成待补充占位或执行假设，除非原始 Prompt 明确要求文档或文件输出，否则不得新增 `docs/`、写文件、持久化等确认要求。输出聚焦“做什么”而非“怎么实现”，不包含实现步骤或代码片段；除非原始 Prompt 提及，不得主动推荐具体技术栈/框架/库，也不得请求教程类产出；每个优化版包含目标/上下文/约束/验收标准四要素，单字段语言纯度与当前界面语种一致，篇幅以 300-1500 字符为宜且不得以未完成列表、悬空连词或冒号结尾
...
- 远端/项目导航与管理入口不得因写能力未认证而消失；每个动作按 owner、project identity、snapshot/plan 与 capability evidence 独立门禁。缺少精确远端项目身份的 project-scope direct inspect/write、legacy writer、未认证 portable 原生 apply、projection create/update/delete、cross-agent apply/full 均 fail-closed。用户级对端三栏走 `agent-hub.user-instructions.v1`，用户级对端资产主列表走 `agent-hub.portable-user.v1`，缺能力不得回落本机 home。`AGENT_HUB_API_VERSION=5` 阻止新旧 GUI/sidecar 混跑绕过该策略（v5 = 用户级镜像写盘）。
- 用户级 Pull/Push 走 `agent-hub.user-mirror.v1` 全 Agent 镜像写盘，必须 preview 后确认；stale plan 禁用 Apply，不得回落旧逐项勾选或 `skipExisting` 冲突策略。缺 `agent-hub.user-mirror.v1` 整次失败。后台自动投影与未认证 portable 原生安装仍受当前 Blocked capability 门禁。
...
- owner/control/Tauri 的 inspect、setup/update preview 与 apply 合同使用短期 plan、CAS snapshot、expected hash、独立 ownership、原子 sibling rename 与幂等存储。可读的本机或在线对端用户级目标允许用户在提示词页面核对合成内容与原始文件后点击“写入原始文件”；对端经 `agent-hub.user-instructions.v1` 在 owning device 执行，不静默写回控制端 home。后端内部生成短期 plan 并立即应用，不额外展示预览 Dialog；后台自动投影、portable mutation 与删除仍受 support manifest/L3 门禁
...
- 提示词三槽（公共 / 适配 / 独有）提供「AI 辅助提示词修改」：用户给出改写方向，本机 Claude Code 只改当前 lane 对应槽（适配覆盖全部 Agent 适配变体），并保存到 Canonical；不自动写入 Agent 原始文件。保存成功后页面显示明确成功反馈；当前可见槽有变化时自动聚焦并选中首处变化区间，只有其他 Agent 的适配槽变化时明确说明当前槽未变
...
- 用户确认关闭 GUI 后必须真正结束桌面进程（拆除托盘并以进程退出，而不是只发 `app.exit` 请求）；macOS 程序坞不得因托盘或未完成的退出请求继续显示该应用在运行
- 独立后端进程保持纯 CLI 身份（不得向 LaunchServices 注册为应用成员），不占据程序坞；仅关闭 GUI 后程序坞应变为未运行，后端继续作为后台进程服务局域网/手机
- 自动发现同一局域网内的其他实例
- 设备上线/下线实时通知
...
**描述**：把用户配置的 GitHub 私有仓库作为中心化同步对端，承载多设备之间的离线数据交换与审计历史。云端同步是内测功能，默认关闭。

**功能点**：
- 须先在设置「内测功能」页打开云端同步，才显示仓库 URL/分支/启用/自动同步配置与手动同步入口
- 内测功能关闭时，后台 scheduler 与手动同步均不执行（即使 `cloud_sync_enabled` 曾为 true）
...
**描述**：监测久坐与可配置健康习惯，在设置里用同一套模板管理饮水、休息、提肛和自定义提醒。

**功能点**：
- 在场时间口径：用户「在场」指处于工作窗口相位（Working）——键鼠活跃，或距上次活跃未超过 `breakSeconds` 的短停歇（看屏幕不动也算在场）；连续无操作达到 `breakSeconds` 即判定离场（Resting / Idle）。活动统计与健康提醒计时都以在场时间为准，用户离开设备的时间不计入
- 久坐监测：每分钟采样前台键鼠活跃度，推进共享工作/休息时钟（Idle / Working / Resting）；`breakSeconds` 只负责关窗，不再单独决定「哪条提醒」
...
- 按在场时间提醒：用户不在场（未处于工作窗口相位）时不弹任何提醒，包括固定间隔模板；固定间隔模板（饮水/提肛等）按在场时间累计计时——离开期间计时暂停，恢复在场后从断点继续，凑满间隔才触发，不对空椅子累积弹窗。键鼠采样不可用（未授权输入监控 / 无图形环境降级）时豁免门控并退化为按墙钟间隔提醒，避免提醒静默失效
...
- 健康提醒页：以状态概览、今日在场指标（在场 / 键鼠活跃 / 离场分钟与活跃占在场比）和按已启用模板渲染的习惯统计展示监控控制台，头部配置入口跳转设置页健康提醒 tab
- 活动统计页：独立侧栏入口，只统计用户在场的时间——展示 app 使用时长排行、窗口标题排行和 24 小时在场分布（键鼠活跃分钟必有窗口信息，排行口径不变）；头部配置入口跳转设置页活动统计 tab
...
- 常规 / 同步 / AI 页签的恢复默认按钮始终可点击；常规恢复为后端按当前设备环境生成的默认设备名、默认接收目录和平台默认截图快捷键，同步页恢复局域网/备份相关默认，AI 恢复 Claude CLI/AI 默认配置。云端同步默认配置的恢复在内测功能页的云端同步区块
- 同步、AI 和关于页签分别管理局域网同步与备份、Claude CLI/AI 能力和应用更新；GitHub 私有仓库云端同步迁入内测功能页。AI 页签中的 CLI 路径与模型供 GitHub 项目解说和 Prompt 优化共用，启用开关与缓存时长仅作用于 GitHub 项目解说；AI 页签同时管理 Workbench Prompt 优化浮层快捷键与自动填入语言，默认轻按 Control、默认填入中文优化版；同步和 AI 的恢复默认只重置表单，仍需用户点击“应用配置”持久化
...
- 内测功能页（`/settings?tab=experimental`）：充电模式、游戏模式、网页浏览、项目自动化、云端同步默认全部关闭；用户须在本页显式打开后，对应入口才出现。充电/游戏/自动化/云同步的设置集中在该页底部独立 tab（只渲染当前选中项）；网页浏览仅总开关、无独立设置 tab。关闭后立即隐藏入口与对应设置 tab，不删除已保存数据。旧深链 `/settings?tab=battery` 与 `/settings?tab=automation` 解析为内测功能页并选中对应设置 tab（`?tab=experimental&feature=battery|automation`）
- 充电模式设置（仅内测功能「充电模式」开启后）：切换充电/无限、展示剩余时间与今日已充/已用、调整闪卡充入分钟与日上限、余额上限、查看流水。健康习惯额度在健康提醒模板上配置。模式与余额以本机 SQLite 为权威，闪卡/余额上限走 `config.json` 的 `battery` 字段；健康字段暂留作旧模板回退
...
- 充电模式（内测功能，默认关闭）：开启后侧栏 footer 主题按钮前出现充电/无限切换器（卫星窗瘦 footer 只保留该按钮与剩余时间）。充电模式下，桌面前台停留在 `/workbench` 或 `/attention` 时按墙钟消耗剩余分钟（主窗与卫星窗同时前台只扣一份）；余额为 0 时仅遮罩工作台 `<main>`，不杀已跑 Agent，不拦 HTTP/CLI/手机 API。健康提醒 completed 按该条模板的充入分钟与日上限入账，记单词答对一张闪卡按电池设置入账。入账 `+Xm` 动画与电池环不得被工作台遮罩挡住。随时切回无限模式，余额保留。余额每日本地时间 8:00 重置为满值（与当前模式无关；应用错过 8 点时在下次读取/扣时/入账时补一次重置，同一天不重复），「今日已充」统计不含每日重置；工作台标题旁徽标随模式切换——无限模式显示 ∞ 标识，充电模式显示剩余可用时长进度条与分钟数（剩余 0 danger、低于告警分钟 warn，悬停显示完整剩余时长）。内测功能关闭时不展示切换器/徽标/遮罩/toast，也不扣时
...
- 工作台顶栏标语：桌面端仅在工作台已选中项目后，在标题区与右侧动作之间的空隙展示设备级全局可编辑标语（仍非 per-project；2026-08 起「现场快照」二级入口不再在标题行展示）；标语在空隙内缩小后尽量靠近窗口中线，但不盖住标题或按钮，不强制绝对居中。支持轻量 Markdown（粗体/斜体/删除线/行内代码/http(s) 链接）与 emoji；单击预览进入编辑，失焦或 ⌘/Ctrl+Enter 保存，Esc 取消未提交草稿；正文落在 owning device SQLite 单行表（上限仍 280 UTF-16），不进 `config.json`、workspace layout、备份可恢复领域或 Git。选中本机项目读写本机标语；选中远端项目经 `POST /api/workbench/banner/{get,save}`（capability `workbench.banner.v1`）读写对端标语。本机首次读空可一次性从 legacy `localStorage`（`cp-workbench-banner`）灌入后停用该 key；缺 capability 为 unsupported，禁止静默再写 localStorage。远端离线只读上次成功值或空并禁用编辑。按区域尺寸二分最大字号使全文可见并尽量撑满，不拉字距、不压扁字形；非工作台路由、零项目/继续工作页、终端全屏、移动端 `/mobile` 不展示。不新增第 8 个 Workbench controller。
- 工作台内置项目预览（内测功能，默认关闭）：设置「内测功能」打开网页浏览后，才可在终端/文件预览旁打开 dev server 浏览器预览，自动发现终端输出和常见框架端口；本机、远端项目 shortcut 与移动端 `/mobile` 均通过 cc-partner 安全代理访问。后端只允许代理 loopback `http(s)` 且显式端口目标，避免成为开放代理；代理请求体上限为 32MB，超限返回 413 且不转发上游；上游响应体必须流式转发，避免超大或持续分块响应在后端整包聚合；远端项目必须在 owning device 上发现和访问目标，当前设备只暴露 relay proxy；前端预览 iframe 必须 sandbox 隔离且不允许 `allow-same-origin`，避免预览项目 JS 同源访问 cc-partner API；候选来源文案由前端按当前语言渲染，不依赖后端 label 字段；内测关闭时桌面工作区切换与手机 Browser 面板隐藏，发现/创建 preview/代理返回未开启
...
- 添加项目文件夹：用户点击项目文件夹区右上角 `+` 后可选择本机项目或局域网设备项目。PC 本机、PC 局域网、手机看主机目录、手机看局域网设备都走应用内目录浏览（PC 本机不再使用系统目录选择器；Settings 等其它 `chooseDir` 场景不变）。浏览当前目录时可新建一层文件夹，创建成功后自动选中，仍须点「打开项目」才进入工作台。新建本身不 `git init`；任何打开为项目的空目录（可忽略 `.DS_Store` / `Thumbs.db` / `desktop.ini` / `.localized`）在目录所在设备上 `git init`（不提交、不写 README）；已有 `.git` 或真实文件则只打开。局域网项目仍保存 remote shortcut，对端创建或复用 local 项目记录，不要求预先添加或授权。浏览层 mkdir 走 `POST /api/workbench/fs/create-dir`（capability `workbench.fs.create-dir.v1`），不回落项目内 `files/create-dir`。
...
- 项目切换：左侧栏项目文件夹列表切换当前项目，中央 worktree/window 列表和右侧文件夹按当前项目刷新；每个项目卡片右下角显示已打开 terminal window 数与 pane 总数，而不是固定“进入工作台”文案；旧项目的异步请求结果不得覆盖新项目 UI。同一桌面会话内再次切回已访问项目时，必须立刻恢复该项目上次 active worktree、已挂载 xterm 与上次 Git 历史，不得先清空画面再等 `git status` / 终端 snapshot replay；后台仍刷新 worktrees/sessions/git log。首次进入某项目仍要拉清单，但可用已返回的 session.worktreeId 先亮终端，不必等 worktrees.list 的 Git 状态。Git 历史刷新不再串行等待 worktrees.list。
- 同一 Git 仓库合并：侧栏按规范化 Git remote（SSH/HTTPS 相同即同一仓库，不限 GitHub；GitHub SSH-over-443 主机 `ssh.github.com` 与 `github.com` 视为同一主机）把本机与各设备上的 clone 合成一项；无 remote 的目录不互并。点刷新扫描 origin 并写回 `gitRemoteFingerprint`（离线/读失败保留上次结果）。列表项展示上次打开的路径并提示其他设备；点该项打开上次使用的工作区（设备筛选开启时取该设备上上次使用的成员）。设备/路径切换在工作区标题下拉里完成，不在列表拆成多条。设备筛选只收窄「哪些仓库出现」。删除多项时弹出勾选。底层仍是一行设备×路径，终端/占用/新窗口仍按原 projectId。
- 跨设备同步主分支：桌面 Git 历史栏与手机 Git 面板的 Commit 右侧提供「同步」。点击后推送当前仓库主工作区（isMain）分支到 origin，再对同一 fingerprint、其他设备上的项目主工作区执行 pull。同机额外 clone 不作为目标。没有其他设备、主分支不能 push、busy 或远端只读时按钮禁用。部分设备失败时保留已成功的拉取并展示摘要。
- 移动端 Workbench：桌面端 HTTP server 默认提供 `/mobile` 普通浏览器入口，面向本机/局域网使用，不要求访问 token；合法 LAN peer 与桌面/native peer 能力相同，UI 必须明示“同一可达网络中的任何设备均可读取、写入和执行；系统不验证调用者身份”，不得写“已认证/可信设备/安全设备”或可切换 LAN 模式。同一浏览器会话内切换已访问项目时，必须立刻恢复上次 worktree/session 与已挂载 xterm，不得先清空再等 `git status`；终端面板与文件面板一样 hidden 保活（离开 terminal 不卸载）；同一会话最多常驻 8 个 xterm（LRU，当前窗口永不淘汰）。`worktrees.list` 切项目关键路径传 `includeGitStatus=false`，后台再补完整 Git 状态。HTTP server 尽量使用固定默认端口，端口被占用时自动向上 +1，避免移动访问链接每次启动随机变化；桌面端全局侧栏左下角颜色模式按钮右侧提供手机按钮，点击后展示可复制访问链接与二维码及固定风险提示，不在 Workbench 右侧检查器内嵌该信息。文件传输是全局 Tools 面板（不绑项目），经 `/api/mobile/devices` 与 `/api/mobile/transfer/*` 走主机中转，已完成 Receive 只提供 Download，选择文件后立即自动开始上传（发送按钮仅作失败兜底）。移动端先加载最近项目列表，并可添加主机本机目录或局域网在线设备目录（应用内目录浏览，无系统目录框；可在当前目录新建一层文件夹后确认打开；本机走 `/api/mobile/workbench/fs/*` 与 `projects/open`，局域网经主机二级代理 `/api/mobile/workbench/remote/*` 写入 remote shortcut）；行尾 ⋯ 可从最近列表移除记录（不删磁盘）；加成功后直接进入该项目工作台；本机项目和远端项目快捷方式都可进入 worktree、terminal、files、git 和自动化面板；主导航不含 Prompt 优化（仅终端内浮层，写入成功短暂提示后自动消失）；终端右下角折叠按钮组在已选项目时即使还没有终端窗口也显示，位置抬到常见 Agent 底部输入框之上，点按不得唤起系统键盘，点开后沿左上象限环形展开（圆钮在弧上、文字沿同角度外侧，带轨道与错开动效），在 Prompt 优化上方提供与桌面 Git 历史相同的 Commit 快捷键（不弹手写 message，后端 `git add -A` 后用 Claude Code 生成提交说明，无可提交改动为 no-op）；提交成功后终端与 Git 面板用 StatusMessage 短暂提示「提交成功」，随后自动消失，全屏同样可见；failedHook 后终端与 Git 面板提供与桌面相同的「让 AI 修复 / 重试 / 忽略」，经 `/api/mobile/workbench/worktrees/repair-hook-failure` 在 owning device 启动可见修复终端；全屏时仍展示 commit 错误与钩子修复卡片，不得把失败藏进已隐藏的外围 chrome；移动端 Settings 只提供外观主题，不检测/安装 tmux；移动端 Inbox 与 badge 隐藏 tmux/依赖条目；远端项目通过 `/api/mobile/workbench/*` 走“手机 → 本机 → 远端设备”二级代理，由本机复用 commands 层 remote-aware helper，再转发到 owning device，避免移动端另写一套远端逻辑；手机竖屏导航默认收起，用户需要时通过顶部按钮打开。软键盘：终端 xterm 进入输入态时，仅当点击位置在 shell（及全屏终端 overlay）按键盘高度整页上移后仍落在未遮挡可视区内才上移；否则键盘直接遮盖页面底部，不上移。其它输入按当前焦点把页面/弹层上移，使输入位于未遮挡可视区纵向中间，原始位置已在该区域上半则保持原位，不得一律顶满。默认 active worktree 优先主工作区，默认 active session 依次优先匹配当前 worktree 且 running、任意匹配当前 worktree、任意 running、首项，并在移动端 shell 状态栏展示当前项目、worktree 和 session；状态栏中 project/worktree/session 均为只读上下文指示。终端面板在窗口 tab 上方提供与桌面 `WorkbenchWorktreeBar` 对齐的 worktree 工作区 tab 列表；files / browser / git 把同一条放在 shell 固定 chrome。点击 chip 切换 active worktree（须过 Files dirty guard），非主 chip 提供关闭并经共享 Dialog 确认后删除，末尾「新 worktree」展开 prefix/suffix 表单；创建成功后自动创建 1 个绑定该 worktree 的 terminal window 并切过去，window 创建失败不回滚 worktree。终端全屏时隐藏面板内 worktree 条与 window tabs。合并仍只在完整 Worktrees / Git 面板。完整 Worktrees 面板创建同样自动开绑定窗口；点击工作区卡片成功切换后直接进入 terminal 面板；删除/合并等破坏性动作只允许非主 worktree 且非 busy。移动端 worktree 状态分类遵循真实 DTO 的 `conflicts > 0` 时 conflict、否则按 clean 判定 dirty/clean。移动端终端面板展示真实 xterm，终端输出区一指上下滑动必须映射为 xterm scrollback 并阻止浏览器页面滚动/地址栏显示隐藏，首屏通过 `/api/mobile/workbench/sessions/replay` 读取历史输出，后续通过 `/api/workbench/events` NDJSON 增量 buffer 更新，输入、resize、focus、新增 pane（固定使用 down split-pane）、switch-pane、close-pane 和关闭 window 均调用 `/api/mobile/workbench/sessions/*` HTTP routes；移动端不暴露左右/上下分屏选择，只提供新增 pane、切换 pane、关闭 pane；终端面板提供全屏模式，全屏时隐藏移动端 shell、标题、worktree 条、window tabs、加载/错误提示等外围内容，只保留 pane 功能行、退出全屏按钮和当前终端输出；且在 focus/新增/切换/关闭 pane 后调用 zoom-pane 幂等保持 tmux window zoom，只显示当前 active pane；不新增第二套 tmux/session/window/pane 模型
- 移动端终端 extra keys：`/mobile` 终端底部固定快捷键条补齐 Esc/Tab/方向/Ctrl 等；短按 `/` 插入 `/`；长按 `/` 后在键上方弹出 `/clear`、`/rewind`、`/resume`、`/compact`（靠近主键往上），按住滑动到目标再松手插入对应文本且不自动回车；松手仍在 `/` 上则插入 `/`，滑出弹出层则取消。方向键（↑↓←→）按下立即发送，按住超过约 400ms 后按约 80ms 间隔连发，松手即停；Esc/Tab/Ctrl 等其它键仍为单击。不宣称真实 iOS/Android 软键盘 L3
- 移动端终端复制与贴图：`/mobile` 终端输出区长按进入自管选区（拖出高亮后底栏复制到系统剪贴板），必须屏蔽浏览器 loupe / 选区手柄，不得把长按交给 xterm helper textarea。贴图主入口是终端右下角操作组展开后最上方的图片按钮，打开系统相册后立刻经现有 `POST /api/mobile/workbench/sessions/paste-image` 发给 owning device Agent；不预览、不裁剪。浏览器 paste 事件中的图片仍可走隐藏通道。不宣称真实 iOS/Android 剪贴板或相册 L3。
- 移动端自动化面板：自动化作为移动端 shell 主导航里的项目级同级面板，入口紧跟项目面板之后，不放在顶部 worktree quick switch 中；选择本机或远端项目后，面板通过 task-view route 读取 local/remote/pendingRemote tagged union，真实任务按 `task.workflowState` 渲染为紧凑 grouped-list，不实现横向拖拽；remote outbox 单独展示，不参与 workflow 分组、selection 或 evidence：pending/sending 只展示待发送状态且无用户动作，failed 提供 Retry/Discard（Discard 需确认）。每个真实任务行展示 workflowState、runState、attemptPhase、lastRuntimeMessage、Claude session/transcript runtime 摘要以及 local/remote 来源，缺失 runtime 字段显示 unknown。点击任务展开详情，详情展示 title、goal、acceptance、workflow/run/attempt、runtime summary、blockedReason、evidence 时间线，并提供“打开执行现场”动作；当任务带 worktreeId/sessionId 时，该动作切换到现有移动端 terminal 面板并聚焦对应 worktree/session，不新增第二套 terminal 模型。创建任务入口是独立弹窗而不是固定内嵌表单，弹窗支持手动填写任务标题、目标和验收标准，也支持输入简单 Prompt 后调用 `/api/orchestrator/tasks/complete-prompt` 让 AI 自动完善三字段；AI 完善只填充表单，用户确认时可选择“创建到 Backlog”“创建到 Todo”或“创建并启动”，HTTP `createView` 请求必须携带非空 `clientRequestId` 并显式传 `createAction: backlog|todo|start`；远端离线时返回 pendingRemote outbox 项并在单独 outbox 区展示，成功后关闭弹窗、清空表单并更新列表，创建成功 status 短暂展示后自动消失；无项目或请求失败时展示对应空态/错误。
- Worktree 管理：每个项目默认有一个主 worktree，路径为用户添加的项目根目录；项目载入时必须读取 `git worktree list --porcelain`，把磁盘上已有的 Git worktree 同步进顶部 worktree 管理层，因此用户选择项目后应立即看到主工作区和已有 worktree 工作区；顶部 worktree 管理层只负责创建功能 worktree、切换 active worktree 和移除非主 worktree。创建 worktree 时点击“新建 worktree”先展开页面内表单，分支前缀从固定类型（如 `feature` / `fix`）中选择，用户只输入后缀，确认后组合为 `<prefix>/<suffix>` 并在应用数据目录下执行 `git worktree add -b <branch>`；创建成功后应自动创建 1 个绑定该 worktree 的 terminal window，默认包含 1 个 pane，并切换到新 worktree/window；如果 window 创建失败，不回滚已创建 worktree，只提示终端创建错误；如果同名本地分支已存在但已合入 base/HEAD，后端应先删除旧分支再重建，未合并则返回可操作错误；不得依赖 WebView 可能不可见的浏览器阻塞弹窗。移除 worktree 时后端自动关闭该 worktree 下所有 terminal window/pane，无需用户先手动关窗口，且不得影响同项目其他 worktree 的窗口。第一期不做 Git diff 面板、交互式冲突解决和 PR 创建
- Git 提交树：右侧检查器提供“项目文件夹 / Git 历史 / 项目笔记”tab；Git 历史 tab 顶部展示当前 active worktree 的 clean/dirty/conflict 状态和 Commit、Pull、Push、Merge 操作，下面只查询当前 worktree `HEAD` 可达的 Git DAG，不混入同仓库其他 worktree 尚未合并的分支提交，切换 worktree 后必须刷新为该工作区自己的历史；提交列表采用类似 VS Code Source Control Graph 的紧凑连续泳道，分支收束时平滑换 lane，以不同节点形态区分 HEAD、merge 与普通提交，并在两行内展示提交摘要、内联 ref badge、短 hash、作者和相对时间，ref badge 区分本地分支、远端分支和 tag；commit 点击后不弹手写输入框，后端执行 `git add -A`、读取 staged diff/stat，并在 active worktree cwd 下用 Claude Code 项目上下文模式生成 commit message 后 `git commit -m`，无可提交改动时作为 no-op 返回最新 worktree 状态；钩子失败的 AI 修复在 owning device 跑同一套 `repair_local_worktree_hook_failure`（capability `workbench.hook-repair.v1`，`POST /api/workbench/worktrees/repair-hook-failure`），控制端 remap `agentSessionId`/`terminalSessionId` 为 `remote:<deviceId>:<inner>` 以便聚焦对端终端，缺 Claude CLI 沿用既有 fail-closed 文案，不再要求用户「请在对端设备执行」；push 优先复用当前分支 upstream 执行 `git push`，没有 upstream 时只选择 `origin` 执行 `git push -u origin <branch>`；完全没有 origin/upstream，或只有 `*-upstream` 等源码上游 remote 时返回配置提示；Git 历史工具条的 Push 按钮只在后端 status 判定当前 worktree 有可用推送目标时启用，本地未发布且没有 origin/upstream 的项目必须禁用；merge 在主 worktree 上：当存在可收集本地分支（排除 home、其他 worktree 占用、已完全合入 home 的分支）时可点；home 探测顺序为 origin/HEAD → 本地 main → 本地 master；后端在隔离 integration worktree 内按分支名顺序 `--no-ff` 合进冻结 home OID，冲突走同一套 Claude Code 隔离解冲突；不关闭主工作区终端、不删除主 worktree；成功后把真实主工作区切回 home 并删除已合并源分支；live 不在 home 时用 CAS `update-ref` 发布再 checkout；live 已在 home 时 `--ff-only`；merge 非主 worktree 时先要求源 worktree clean，有未提交改动立即返回可读错误；源 clean 后后端自动关闭该 worktree 下所有 terminal window/pane，再检查主 worktree clean 并冻结主分支、主 HEAD OID 与源 worktree 实际 HEAD OID；若冻结源 HEAD 已是冻结主 HEAD 的祖先（含同一提交），视为已合入：跳过隔离 merge 与 Claude，直接清理源 worktree（session/磁盘/已合并分支/SQLite row），主工作区 HEAD 不变；owner 重启时同一 `clientOperationId` 按该祖先关系视为已发布并幂等清理；否则后端在应用数据目录的内部 detached integration worktree（不写入 SQLite、不出现在用户 worktree 列表，且必须位于 main/source checkout 之外以避开 watcher）基于冻结主 OID 执行 `git merge --no-ff <source_oid>`，真实主 worktree 在隔离阶段保持不变；merge 冲突时调用本机 Claude Code CLI（设置页 AI 的 CLI 路径和模型）在该隔离 worktree cwd 下以项目上下文 headless 模式处理：prompt 只传冲突相对路径，不再内嵌全部文件全文或要求完整 JSON 回传；CLI 只开放 Read/Edit/Write/Grep/Glob，不开放 Bash，dontAsk 权限仅预批准隔离 worktree 根内读写和只读搜索，CLI 返回后由 Git 强制限制最终改动只能落在原冲突路径；解冲突进行中 `resolveConflicts` 阶段在状态文案下展示 Claude Code 最新一行输出动向；超时仅在连续 300 秒无 CLI 输出时触发，有输出则继续等待；关终端、隔离 git merge、发布和 cleanup 等非 Claude 阶段各自有 120 秒墙钟上限；桌面 control、P2P 远端与 `/mobile` merge HTTP 等到对端返回；Claude 按需读取上下文并直接编辑冲突文件，返回后后端拒绝原冲突清单外的 unstaged/untracked 改动，复查所有 marker 已移除，再 stage all 并完成 merge commit；失败只 abort/清理隔离目录，不污染真实主工作区；产物必须严格为 parents `[main_oid, source_oid]` 的双父 merge commit，发布前再次确认真实主 worktree 的分支、HEAD 与 clean 状态，以及源 worktree 的分支、HEAD 与 clean 状态仍与冻结快照一致，然后以 `--ff-only` 短时发布；main 或 source 已漂移/变脏时拒绝发布并保留源的新提交或改动；若 owner 在发布后、cleanup/ledger 成功落盘前重启，同一 `clientOperationId` 必须按冻结 intent 精确确认已发布 merge，幂等完成隔离目录清理，在源仍匹配冻结输入时清理源 worktree，并收敛为 succeeded，不重复调用 Claude；只有发布成功且源仍未漂移时才删除该 worktree session 元数据、Git worktree 本身和已合并本地分支；merge 命令保持返回 `{ok, worktreeId, stages}`，并通过 `workbench:merge-progress` 事件按 `checkSource/closeSessions/mergeMain/resolveConflicts/cleanup` 阶段推送 `pending|running|completed|failed|skipped` 状态，事件 payload 带 `projectId`；前端必须按项目缓存正在运行的 merge 阶段，切换 project 或 worktree 时继续接收后台事件，返回原项目后恢复最新阶段，且 command 终态不得因 active worktree 已变化而被 stale guard 丢弃；阶段条在成功完成后自动隐藏释放 Git 历史空间，失败时保留错误阶段；切换项目/worktree、commit、push 或 merge 后刷新，commit 失败也应刷新当前 worktree 状态，空仓库或无提交时显示空态
- 项目笔记：桌面右侧检查器第三 tab 提供当前 Workbench 项目一份 Markdown 笔记（切 worktree 共用）；复用现有 Tiptap 富文本所见即所得，500ms debounce 写入 owning device `~/.cc-partner/data.db` 的 `workbench_project_notes`（主键为对端 inner local project id）；不写仓库文件、不进 Git、不 LAN/GitHub 同步、不进备份域。远端项目经 `POST /api/workbench/notes/{get,save}`（capability `workbench.project-notes.v1`）读写 owning device，控制端 remap 返回的 `projectId` 为 shortcut id；owner 拒绝 `remote:` 递归。不迁移已写在控制端 `remote:…` 下的旧行（避免双源，旧行忽略）。离线远端禁止再写本机 shortcut。删本机 shortcut 只级联清本机孤儿行，不删对端笔记；对端笔记随对端 local 项目删除。关 GUI 前经 pendingWrites flush。第一版 `/mobile` 不提供笔记面板
- 设备级全新启动连接：工作台状态卡可对当前设备（本机或局域网远端）触发「全新启动连接」——两阶段确认（弹窗标明目标为本机或远端设备名；预检会话清单/非工作台 tmux 会话数/引导通道可用性）后，关闭该设备全部工作台终端会话并以全新登录环境重启 tmux server，使系统级变更（用户组、limits、profile）在新终端立即生效；新建终端 pane 默认为 login shell。存在用户自己的 tmux 会话时须显式勾选才会一并终止；loopback ssh 自动引导不可用或执行降级时，结果中提供可复制的手动命令（`cc-partner-backend workbench-fresh`，在任意新 SSH 登录中执行；后端运行中会拒绝该子命令并引导使用应用内入口）。本机与远端行为一致（capability `workbench.fresh-restart.v1`；缺能力显示 unsupported）；桌面 GUI 本机路径也经 sidecar owner 执行，不在 GUI 进程直连 tmux。
- tmux 依赖管理：应用启动后自动检测 Workbench 所需 tmux；macOS/Linux 使用原生 tmux，Windows 使用默认 WSL 发行版内的 tmux。状态卡应展示 checking/ready/missing/installing/unsupported/failed 状态、后端、版本、路径、安装命令预览和最近输出；缺失且存在可用安装通道时用户可确认安装，安装完成或失败后可重新检测。Settings 依赖页只管本机；`/mobile` Settings 不展示 tmux 依赖检测/安装。选中远端项目时 Workbench 依赖卡展示对端状态，确认框写明设备名与将在对端执行的精确 argv（capability `workbench.dependency-install.v1`，headless 可 `spawn_install`，不依赖 GUI）。Linux/WSL `sudo` 无 TTY 超时后 fail-closed（「无 TTY 无法输入密码，请在对端本机执行 doctor 提示的命令」），禁止挂死等密码。缺 capability 或旧路由时卡片为 `unsupported`（Workbench 不占位），不假装装成功，也不把「无法探测」误报成 tmux 缺失；只读 status 在对端未宣告 token 时仍 best-effort 探测，对端已装 tmux 则按 ready。Attention 本轮仍只投影本机 tmux，不把对端缺失刷进 Inbox。LAN 风险与现有「远端终端可写任意命令」同级，不新增鉴权。tmux 不可用时仍允许普通 PTY fallback，但不承诺 window/pane 语义和重启后 shell 上下文恢复
...
- tmux 冷恢复历史：应用重启后恢复已有 tmux window 时，后端必须在重新 attach 前用精确 window target 捕获 tmux 历史区（不含当前可见屏幕并保留文本属性），将结果按 xterm 原始换行约定写入当前 generation 的独立有界恢复前缀，再接续 attach 对当前屏幕的重绘和实时输出。恢复前缀最多保留最近 80k 字符且不参与 120k live ring 的头部淘汰，组合 replay 不超过前端 200k buffer 合同；不得把历史作为 seq=0 普通 live chunk 写进同一 ring，否则活跃 TUI 的高频末屏重绘会在几秒内再次挤掉全部历史。前端首次挂载时应以 session 持久化 cols/rows 为已上报基线，FitAddon 结果相同则不得再次 force resize；仅真实尺寸变化、可见性恢复或用户点击“适应尺寸”才允许强制 redraw。捕获失败时允许降级为仅恢复当前视口，但不能阻断 session attach。
- 桌面终端滚轮分流（覆盖上一条中的旧 alternate-only 表述）：应用启用 mouse tracking 时（包括 Claude 以 normal buffer 承载虚拟 transcript）必须拦截滚轮并固定向 transcript 左上发送 SGR 64/65；若 normal buffer 已有后端注入的真实本地历史（`baseY > 0`）且 xterm mouse mode 为 `none`，即使 Agent Runtime 仍为 `isActive=true` 也必须优先使用 xterm 本地 scrollback。Claude `resume` 后若活跃 Agent 的 mouse mode 为 `none`，且处于 normal/`baseY=0` 或 alternate buffer，首次向上滚动必须单飞请求真实 session owner 捕获“tmux scrollback + 当前 pane”的已渲染快照，以 generation-bound 快照替换旧 replay prefix/live 控制流，使 xterm 最终落在可滚动 normal buffer；普通启动 baseline、Gap resync、增量读取和 presence probe 不得触发 `capture-pane`。前端等待 xterm `onWriteParsed` 后复核同 session/Agent、normal buffer 与 mode，再应用首次累计滚动；失败或超时必须解锁以允许重试。只有显式 mouse tracking 的 alternate screen 走固定 SGR。禁止使用指针坐标、PageUp 或方向键。
- 移动端终端滚动恢复：`/mobile` 每份新 xterm 在 mouse tracking 未启用时，首次向历史方向滑动必须等普通首屏 replay 完成，再单飞请求同一 replay route 的显式 `refreshHistory=true`，把真实 owner 的 tmux history hydration 到本地；普通首屏、Gap resync 和轻量探针不得触发 capture。请求期间累计本次滑动并按 owner/seq 合并实时输出；权威快照必须以独立 `historyHydration` reset 完整替换本地屏幕，alternate buffer 先在本地 reset 回 normal，解析完成后复核同 session/generation/Agent、normal buffer、mouse none 与 `baseY>0`，再通过绝对 `scrollToLine` 执行原滑动。失败、超时、外部 reset、换 session 或模式变化不得卡死后续手势，旧响应不得覆盖新终端；每份新 xterm 必须重新建立 hydration 权威性，不能信任上一实例或末屏重绘污染出来的 scrollback。手机浏览器从后台回到前台时（`visibilitychange` 可见 / `pageshow` / `online`），必须立刻 abort 半开的 `/api/workbench/events` NDJSON 并以 after 游标重连（或走 Gap resync），同时重建输入 WebSocket 且不得重放未 ACK 输入；不得等待被冻结的 35s watchdog。当前项目工作台必须把 `projectId/panel/worktreeId/sessionId` 写进 `/mobile` 地址栏（回到项目列表则清 query）；刷新或进程被杀后按 URL 直接打开原来的工作台，项目/窗口已不存在时回落列表，不得让用户从项目列表再点一次。终端必须用绝对 `scrollToLine(baseY)` 显示最新输出，并在随后一小段 pin 窗口内跟随 catch-up；划选复制中或用户再次手势滚动时不得抢滚动。不得停在离开时的旧视口让用户手动滚到底。
...
- 会话状态：右侧只展示当前 window 状态，包含设备、项目名、worktree 名、工作区路径、window 名、命令、状态、运行时长、尺寸、开始时间和退出码；token 速率与上下文占用绑定当前 native session，`/clear` 或新对话后不得继续显示上一轮用量
- Agent 自动标题：用户在 Workbench 普通终端内与 Claude Code、Codex 或 OpenCode 对话时，window/tab 名应跟随 provider 生成的真实会话标题；Claude transcript watcher 随 running terminal/worktree 启动，不依赖用户先打开历史搜索。标题在 terminal Ready/native/pane 绑定暂未完成时必须保留并重试，同一标题只有在成功落地、已经同名或被用户手动命名锁定后才能去重；没有 native session 强绑定而使用 cwd 兜底时，标题来源更新时间不得早于当前 window 启动时间，防止已关闭会话的旧标题污染新窗口；多 terminal 同 cwd 或多 pane owner 不明确时继续 fail-closed。用户 `/clear` 或开启新对话后，window/tab 名必须跟到新会话：尚无实质标题时退回默认名，生成新标题后再覆盖，不得继续显示上一轮主题。自动标题更新必须通过本机桌面、远端桌面和移动端事件链实时投影，Gap 后用 session inventory 对账，无需重连或重新进入页面。
...
- 项目文件夹：右侧文件树绑定 active worktree 根目录，支持刷新、展开/收起、选中文件/文件夹、新建文件、新建文件夹、重命名、删除确认、复制相对路径，并展示名称、类型、相对路径、大小、修改时间和父目录；点击文件节点在中心文件工作区打开 tab，预览 tab 标签直接显示文件相对路径，格式化/保存按钮与 tab 同行显示，不再渲染第二行文件头部或类型标签；支持多文件 tab 激活/关闭，全部关闭后留在空白文件页；worktree 行最右的文件浏览段始终可点，无打开文件时进入空白页并切右侧栏到项目文件夹；用户可在终端、网页浏览和文件工作区之间切换；项目或 worktree 切换时清空文件工作区，旧异步请求不得污染新上下文；重命名/删除路径后已打开 tab 必须同步新路径或关闭
...
- 终端输入传输（覆盖上文中“输入走 sessions HTTP routes”的旧描述）：桌面 xterm 与 mobile xterm 的交互输入固定走 `cc-partner.terminal-input.v1` 常驻 WebSocket；桌面 invoke 只等待 GUI Rust 有界队列接纳，mobile 使用同源 `/api/mobile/workbench/terminal-input-stream`，本机到远端 owning device 使用 `/api/workbench/terminal-input-stream`。ACK 仅在 PTY write+flush 后返回且不阻塞下一帧发送；断线时已发送未 ACK 输入结果未知、不得自动重放；两个 xterm 入口不得回退 `/sessions/write`。终端输出继续使用既有 NDJSON/replay/gap 通道。Agent TUI（Claude Code / Grok 等）图片粘贴不走该输入流：本机拦截 paste / Ctrl+V 中的图片，经 `POST /api/workbench/sessions/paste-image`（capability `workbench.terminal-paste-image.v1`，mobile 为 `/api/mobile/workbench/sessions/paste-image`）把 PNG 写到 owning device 系统剪贴板并注入 Ctrl+V，使对端 Agent 读到图；owning device 没有可达的 X11/Wayland 剪贴板（headless / SSH 过期 forwarding）时，把 PNG 写到该设备 `{data_dir}/tmp/paste-images/`，再按身份表 `headless_image_paste` 注入该 Agent 的路径语法（Claude/OpenCode/Grok/Gemini/Cursor 为 `@绝对路径`，Codex 为 bracketed path paste，Pi 为绝对路径），不发 Ctrl+V。缺 capability 为 unsupported，禁止把数 MB 图片塞进 32 KiB 输入帧。移动端主入口是折叠 FAB 展开后的相册，不是长按系统粘贴。桌面工作台还支持把 Finder 拖入或 Cmd+V 粘贴的非图片文件交给 Agent：本机项目注入原生路径；远端项目经 `POST /api/workbench/sessions/attach-files`（capability `workbench.terminal-attach-file.v1`）把文件树写到 owning device `{data_dir}/tmp/agent-files/`（不进 git / 不进传输接收目录）再按同一身份表路径语法注入；单次解码后总量 ≤20 MiB。禁止把文件字节塞进 32 KiB 输入帧。本轮不做移动端文件拖入。
...
**功能点**：
- 任务队列按项目隔离，支持按 Backlog / Todo / In Progress / Human Review / Rework / Merging / Done / Canceled 固定业务泳道展示任务；**Lane + 仅出现在 Backlog 与 Todo**，打开独立创建 Dialog，用户必须显式点 `createAction`（backlog/todo/start），不得隐式默认。桌面端拖拽只允许移动到前一个或后一个相邻泳道；Backlog/Draft 拖到 Todo 视为入队（legacy Queued + Todo/Idle，与 `queue_task`/`createAction=todo` 相同），空闲非堵塞任务拖到 Rework 同样入队（不必再点「请求返工」），Todo 拖回 Backlog 退回 Draft；拖拽本身不隐式启动 Runner 或交付；本机与在线远端真实任务均可拖（远端经 `POST /api/orchestrator/tasks/move-workflow-state`，capability `orchestrator.move-workflow-state.v1`，owner 只接受 local 项目，pending outbox 拒绝）；`/mobile` 不实现横向拖拽。scheduler 只自动领取本机项目中由已解析 workflow 标记为 active 且运行态为空闲或阻塞的任务，内置默认 active states 为 Todo/Rework，项目 `WORKFLOW.md` 可在固定枚举泳道内覆盖 active states，但不能创建任意新泳道
- **任务块**：2–8 个成员串行共享同一 worktree 与一条 `agent/<block-short>-<slug>` 分支；同一时刻最多一个成员 running。verifier 通过时 **live** 重读 `max(block_index)`：中间成员直接 Done（跳过 Human Review 与 merge main），仅当前最后成员走既有 auto-delivery 或 Human Review。块卡片落在 head 泳道（第一个未完成成员）；成员不得再以独立卡片出现在其它泳道。末尾追加仅当 head 为 backlog/todo/inProgress，拒绝 humanReview/rework/merging/delivering，上限 8。重排仅当全部成员仍 backlog 或 todo 且 idle。Lane 拖拽整块：仅当全部成员仍 backlog 或全部仍 todo。不复用 experiment 的 `delivery_suppressed`。capability `orchestrator.task-blocks.v1`。
...
- **统一 Agent session runtime（A1）**：owning device 维护 provider-neutral `workbench_agent_sessions`（phase/version/关联 ID）；普通 Workbench 与 Orchestrator Runner 共享同一权威。terminal 注入非敏感 `CC_PARTNER_{PROJECT,WORKTREE,TERMINAL_SESSION,OWNER_INSTANCE}_ID`；adapter 经 app-private OSC 上报 phase（单帧 ≤16KiB、每 terminal ≤20 events/s，帧从可见输出剥离）。普通 Claude 交互终端在 auto-title 精确绑定后，owner 以有界轮询消费 Claude metadata-only session 状态（`busy`→working、存活 `idle`→needsInput、连续确认退出→disconnected）；仅接受 interactive/native 精确匹配且不覆盖 Orchestrator，扫描异常保持原 phase，不读取 transcript/terminal bytes。投影 DTO/事件/P2P snapshot **禁止** `nativeSessionId`、Prompt、terminal bytes、transcript path 与凭据；能力 token `workbench.agent-runtime.v1` 仅协议协商。Gap 后取 active snapshot（≤1000）再排空事件；旧 peer 无能力显示 unsupported，不回退猜测 Claude session。同一 terminal 更换 Agent 时新 Agent 的 version 从 1 重新开始，客户端只能在同一 agent id 内比较 version，并优先 active 新行。Orchestrator 一个版本 dual-write 旧 `claude_session_id`/`runtime_*`，completion 先更新 Agent runtime 再进验证
- **Agent 状态投影（A2）**：Desktop/Mobile 消费 A1 runtime（非 Orchestrator 旧 Claude 字段）自动展示 provider 短标签 + phase（listener-first handshake，Gap/owner 重建）；Workbench 项目、worktree 与 terminal tab 的 `等待/已停止` 数字以当前权威 terminal session 清单为统计边界，未读终态可跨刷新保留，但 terminal 关闭或从权威清单消失后必须立即剪除，三层不得出现孤儿计数分叉；`needsInput`/`failed` 进入 Attention v2 与默认 OS 通知；working/idle/completed 默认不进 Inbox、completed 通知默认关；Attention/通知只导航不执行输入/审批/retry
- **Agent Metadata Ledger（A9）**：Agent runtime 终态自动写入 metadata-only 历史（`agent_session_id` 唯一；可靠 usage 可 null-fill；失败不阻断 runtime）。终态时按 provider 从 CLI 本地会话文件提取可靠 usage：Claude（`~/.claude/projects` session JSONL，按 message.id 去重求和）、Codex（`~/.codex/sessions` rollout 最后一个 token_count 累计值）、OpenCode（opencode.db message 表求和）；交互式 wire id 含 `claudeCodeVisible|codexVisible|openCodeVisible` 及历史短码 `codex|opencode`；失败/未提取到保持 null。工作台右侧「当前会话」卡的输入/输出速率**优先**绑定 Agent runtime 投影上的 live usage（owner 每 2s 有界抽取 CLI session 文件，不升 CAS version、不发 OS 通知），缺 live 时回退终态 ledger。速率分母对齐 ccstatusline-zh：用户请求→助手回复的有效生成区间（可重叠合并），不是墙钟。首 token 平均耗时：用户发出指令到本轮第一条助手回复首次写入 transcript 的平均等待（jsonl 无流式首字时间，不含 tool_result 回环）。缓存命中率与 Token 统计页同一口径：`cache_read / (cache_read + input)`。上下文用量只展示当前窗口占用（末轮 `input + cache_read + cache_write`，单位以 k 为主），**不得**把累计计费 token 当占用；上下文长度单独展示模型最大上下文（provider 上报、`[1M]` 提示、已知表含 grok-4.6-build，否则 200k）。缺 occupancy 时用量显示「未提供」。投影/事件不得带 native_session_id/path/prompt/transcript；null 显示「未提供」不显示 0。默认保留 30 天且每设备最多 10,000 条，启动/每 24h 批删 ≤500。本机 drawer 分页（默认 50/最大 200）可滤 project/provider/outcome/time；明细行主标题为**终态时采集的工作台终端窗口标题**（经清洗，无标题回退 providerId）；token 展示分三项：**缓存输入（cache read）/新输入（input）/输出（output）**（>5k 缩写为 k、>=1M 为 M，3 位小数），时长按天/时/分/秒自动划分精确到秒，时间按本机时区显示到秒；unknown token/cost 显示「未提供」（不显示 0）。独立 **Token 统计页**（`/token-stats`）复用同一 ledger：SQL 聚合 cache_write / real_consumed / cache_hit_rate、by_model/by_provider/by_project 与 hour/day 趋势，并支持 CSV/JSON 导出。P2P 仅 `POST /api/workbench/agent-ledger/summary`（capability `workbench.agent-ledger-summary.v1`）暴露 24h/7d/30d aggregate（含 cache read 合计），**无** entry 列表/session id/prompt/path。Fleet 详情 join 7d Agent activity（field 失败不阻断其它摘要；旧 peer unsupported 不显示 0）。Settings 一键清除只删 ledger；不进入 Prompt/SSH/Scratchpad/GitHub sync。
- **LAN Agent Fleet（A6）**：控制设备仅聚合**已保存**的 local/remote project shortcut（不枚举对端全部项目）；按 owning device 批量拉取 Agent phase 计数、Attention、terminal、Git clean/dirty/conflict、browser Active/Absent、Orchestrator running/retrying 与 **device-global** scheduler slots（禁止用当前 project `slotsUsed` 推导）。P2P owner batch `POST /api/workbench/lan-fleet/snapshot`（capability `workbench.lan-fleet.v1`，≤100 projects/device，禁止 remote: 递归）；控制侧 fan-out 并发≤3、单设备 5s timeout、进程内 display cache + freshness；可见 30s safety reconcile。Project Rail 仅 Agent 状态点（needsInput/failed 红点）与 offline 文本（无异常数字 badge、无 Fleet 二级入口）；无独立 Fleet 详情页（Settings 已去掉该 tab）；Project Rail / Mobile 摘要只读且禁止调度/迁移/复制 repo/改 concurrency/inline mutation；DTO 不含 Prompt、terminal bytes、远端绝对 path。mDNS/capability 只表达可达与协议，不称认证/可信/安全。
- Claude Code 开发 Prompt 必须要求完成代码、测试/验证和必要证据说明后，最后单独输出 `ORCHESTRATOR_DEV_DONE`；后端监听可见开发终端输出，只在检测到换行终结的独立哨兵行且该 session/attempt 仍是任务 active runner 后自动进入既有验证/交付流程。手动“完成 Agent 运行”是项目作用域入口：本机走既有 control/owner pipeline；在线远端经 `POST /api/orchestrator/tasks/complete-agent-run`（capability `orchestrator.complete-agent-run.v1`，约 360s，`no-transport-retry`）在 owning device 执行同一套 Running→Verifying + 验证/交付；前端按可行动且在线展示按钮，不再按 local-only 门闩。Owner 上 sentinel 自动完成路径保持不变
...
**描述**：从 Workbench 终端里 Claude / Codex / OpenCode 的 assistant 英文输出沉淀本机词库，并在侧栏版本号旁用半透明大厅打开艾宾浩斯闪卡复习。权威设计见 [`docs/superpowers/specs/2026-08-14-wordgame-design.md`](superpowers/specs/2026-08-14-wordgame-design.md)。
...
- 内测功能「游戏模式」开启后，可在内测功能页配置游戏插件目录，默认 `~/.cc-partner/plugins`；每个一级子目录一份 `game.json` + 入口 HTML（静态或已构建产物）；宿主不执行 npm
- 大厅提供可一键复制的 AI 游戏规范 prompt；插件在全应用半透明 scrim 沙箱 iframe 中运行
- 插件 `complete` 按清单 `rewardMinutes` 入账，无日上限，余额仍受上限钳制；记单词闪卡入账不变
- 七种题（英→中、中→英、选义、语境填空、近义词、搭配、例句改错）由内部 Claude provider 一次生成并缓存；每种题累计答对 2 次后该词熟悉，答错只清当前题型
- 生词按天可出；熟悉后间隔 1/2/4/7/15/30 天；某题型当天答对 2 次后当天不再出该题型
- 连续出题不得立刻再出刚出过的同一词同一题型；中→英题面不得展示原词标题
- 启动预热词频最高的 10 个生词；严格满 10 个生词且题缓存齐全才能进入；生成失败必须重试成功、不换模型
...
- Workbench 无项目时仅聚焦空态 CTA（添加本机/连接远端/检查 tmux）；有项目未选中时展示“继续工作”启动页；Mobile 导航双模式：global=Projects/Inbox/Tools/System，project=Workbench 工具 + Shortcuts（传输全局，项目绑定入口仅 project 模式；Prompt 优化不进主导航）
