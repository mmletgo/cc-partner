...
**描述**：以项目文件夹为中心管理 Git worktree、普通交互式终端、当前工作区文件夹、文件内容浏览/编辑和 Git 提交树。当前已支持本机目录及已挂载局域网目录；后续远端项目能力按可信局域网直连模式扩展，用户可直接浏览已发现设备的目录并选择远端项目文件夹，文件、Git、worktree、终端和 Prompt 优化均代理到远端设备执行。
...
- 当前仍不做 Git diff 面板、交互式冲突解决、会话日志持久回放和批量同步副本；远端 cc-partner 原生项目和远端 PTY 按 `docs/superpowers/specs/2026-06-26-workbench-remote-projects-design.md` 的可信局域网直连方案推进
