...
- macOS 权限引导与设置页共用 `usePermissions`：首轮检查失败结束 loading 并显示错误 +「重新检查」；刷新失败保留 stale 状态；Request 与 Open Settings 是独立动作。输入监控精确呈现 `granted / denied / notDetermined / unavailable`：仅固定内部签名主体允许 Request，denied 才打开设置，社区/ad-hoc 构建显示构建说明且不打开空列表；禁止私有 TCC、系统 reset、pending marker、运行时重签和自动重启
