...
- 远端设备可只启动独立后端 CLI 暴露 P2P/Workbench/Orchestrator 远端支持，无需完整启动 GUI
...
- 远端后端运行时：远端设备的 P2P HTTP、文件传输、Workbench session/event、Git/worktree 和 Orchestrator scheduler/runner/delivery 路径必须能在独立后端进程中运行，业务层不得依赖 GUI AppHandle；GUI 模式只负责把后端事件转发给前端和管理后端进程生命周期
