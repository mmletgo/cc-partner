...
**描述**：以项目文件夹为中心管理 Git worktree、本机普通交互式终端、当前工作区文件夹和 Git 提交树。第一期支持本机目录及已挂载局域网目录；远端 cc-partner 设备上的原生远端项目、远端 PTY 和文件内容预览后续扩展。

**功能点**：
- 工作台布局：项目文件夹列表紧跟全局左侧栏“设置”菜单项下方，作为进入工作台的入口，不再设置独立“工作台”主导航项；主区域依次展示工作台标题、terminal sessions 标识、worktree 管理层、依赖提示槽、当前 worktree 的 window tabs 和终端面板；右侧检查器承载当前 window 状态，并提供当前 worktree 文件夹 / Git 提交树 tab，窄宽下排到首屏终端之后
