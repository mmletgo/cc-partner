...
- 桌面终端滚轮分流（覆盖上一条中的旧 alternate-only 表述）：应用启用 mouse tracking 时（包括 Claude 以 normal buffer 承载虚拟 transcript）必须拦截滚轮；应用冷启动重新 attach 时 tmux 不会向新 xterm 重放 Agent 先前已经开启的 DECSET mouse mode，因此 Agent Runtime 对该 terminal 的最新权威投影仍为 `isActive=true` 时，即使 xterm 报告 mode `none` 也必须按活跃 Agent 处理。两种情况都先退出 xterm 本地重绘历史，再固定向 transcript 左上发送 SGR 64/65；只有未启用 tracking 且没有活跃 Agent 的 normal buffer 才使用本地 scrollback，alternate screen 始终走固定 SGR。禁止使用指针坐标、PageUp 或方向键。
