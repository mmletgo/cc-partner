...
- 设置页健康提醒 tab：以「健康提醒 / 提醒方向 / 免打扰 / 通知与隐私」四个分栏目 Card 展示完整配置表单
- 完整配置表单：工作窗口/休息时长/通知开关/全屏遮罩开关/记录窗口标题/喝水开关与间隔/免打扰起止 24 小时制时间选择器/数据保留天数，全部字段持久化（updateConfig 整体覆盖回写）
...
**描述**：提供集中偏好设置入口，管理基础配置、权限、快捷键、同步、AI 解说和版本更新。

**功能点**：
- 常规设置包含设备名、文件接收目录、macOS 权限状态和截图快捷键
- 截图快捷键通过只读录制控件修改，保存时不应覆盖未修改的设备名或接收目录
- 常规 / 同步 / AI 页签的恢复默认按钮始终可点击；常规恢复为后端按当前设备环境生成的默认设备名、默认接收目录和平台默认截图快捷键，同步和 AI 分别恢复为后端定义的云端同步默认配置与 GitHub Trending/Claude CLI 默认配置
- 同步、AI 和关于页签分别管理云端同步、GitHub Trending/Claude CLI 解说和应用更新；同步和 AI 的恢复默认只重置表单，仍需用户点击“应用配置”持久化
