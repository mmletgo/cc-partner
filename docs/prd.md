...
- 移动端终端 extra keys：`/mobile` 终端底部固定快捷键条补齐 Esc/Tab/方向/Ctrl 等；短按 `/` 插入 `/`；长按 `/` 后在键上方弹出 `/clear`、`/rewind`、`/resume`、`/compact`（靠近主键往上），按住滑动到目标再松手插入对应文本且不自动回车；松手仍在 `/` 上则插入 `/`，滑出弹出层则取消。不宣称真实 iOS/Android 软键盘 L3
