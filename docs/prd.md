...
- 终端输出：PTY 输出必须保留中文与符号等 UTF-8 文本完整性；前端 xterm 必须按 PTY/tmux 原始控制序列渲染，不能启用会改写换行的 `convertEol`；后端启动 Workbench PTY 客户端时必须显式设置 `TERM=xterm-256color` 和真彩色环境，不能继承 `TERM=dumb`；切换 app tab 时必须保留所有 terminal window 的 xterm 实例，只隐藏非活动 window，不能卸载后 replay 原始终端流，同时必须同步底层 tmux current window；前端应轻量同步 tmux current window 到顶部 app tab，不能因为保留 tmux status bar 而让两层 window 选中态分裂；buffer 截断导致必须 replay 历史输出时，不得把 xterm 生成的设备能力响应写回 PTY；创建终端时应使用当前终端可见 viewport 的真实 cols/rows 作为 PTY 初始尺寸，终端运行时 resize 也必须按同一可见 viewport 计算，避免交互式程序状态栏和命令行内容出现替换字符、首屏错位或内容超出工作台可视宽度
- 终端分屏：中央终端区每次 attach 当前 window；左右/上下分屏调用 tmux `split-window -c <项目根目录>` 创建真实 pane，新 pane 必须从项目根目录启动而不是继承当前 pane 的 cwd；关闭 pane 多 pane 时调用 tmux `kill-pane` 关闭当前 active pane，最后一个 pane 被关闭时应关闭所属 window 并同步移除顶部 tab，不应弹“只有一个 pane”的错误
- 会话状态：右侧只展示当前 window 状态，包含设备、项目名、项目路径、window 名、命令、状态、运行时长、尺寸、开始时间和退出码
