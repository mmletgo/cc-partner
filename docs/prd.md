...
- 设备级全新启动连接：工作台状态卡可对当前设备（本机或局域网远端）触发「全新启动连接」——两阶段确认（预检会话清单/非工作台 tmux 会话数/引导通道可用性）后，关闭该设备全部工作台终端会话并以全新登录环境重启 tmux server，使系统级变更（用户组、limits、profile）在新终端立即生效；新建终端 pane 默认为 login shell。存在用户自己的 tmux 会话时须显式勾选才会一并终止；loopback ssh 自动引导不可用或执行降级时，结果中提供可复制的手动命令（`cc-partner-backend workbench-fresh`，在任意新 SSH 登录中执行；后端运行中会拒绝该子命令并引导使用应用内入口）。本机与远端行为一致（capability `workbench.fresh-restart.v1`；缺能力显示 unsupported）。
