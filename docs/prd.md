...
**描述**：以项目文件夹为中心管理 Git worktree、本机普通交互式终端和当前工作区文件夹。第一期支持本机目录及已挂载局域网目录；远端 cc-partner 设备上的原生远端项目、远端 PTY 和文件内容预览后续扩展。

**功能点**：
- 工作台布局：项目文件夹列表紧跟全局左侧栏“设置”菜单项下方，作为进入工作台的入口，不再设置独立“工作台”主导航项；主区域依次展示工作台标题、terminal sessions 标识、worktree 管理层、依赖提示槽、当前 worktree 的 window tabs 和终端面板；右侧检查器仅承载当前 window 状态与当前 worktree 文件夹交互，窄宽下排到首屏终端之后
...
- Worktree 管理：每个项目默认有一个主 worktree，路径为用户添加的项目根目录；顶部 worktree 管理层用于创建功能 worktree、切换 active worktree、查看 clean/dirty/conflict 状态，并提供 commit、push、merge 到主工作区和移除非主 worktree 操作。创建 worktree 时在应用数据目录下执行 `git worktree add -b <branch>`；commit 执行 `git add -A && git commit -m`；push 优先复用当前分支 upstream 执行 `git push`，没有 upstream 时选择 `origin` 或唯一 remote 执行 `git push -u <remote> <branch>`，完全没有 remote 或多 remote 且无 origin/upstream 时返回配置提示；merge 前必须要求源 worktree 和主工作区均 clean 且无冲突；移除 worktree 前必须先关闭该 worktree 下的 terminal window。第一期不做 Git diff 面板、交互式冲突解决和 PR 创建
...
- 终端窗口与 pane：每个工作台项目优先对应一个真实 tmux session，前端 tab 对应 tmux window，window 内分屏对应 tmux pane；运行期 attach 必须在连接项目 session 后切到对应 tmux window，切换 app tab 时必须让后端执行 tmux `select-window` 切到 tab 绑定 window，用户通过 tmux 底部 status bar 或快捷键切换 window 时顶部 app tab 也必须跟随真实 tmux current window，确保 app tab 与 tmux window 一一绑定；没有 tmux、WSL 路径不可转换或后端不可用时退回单普通 PTY window。新建 window 在 active worktree 根目录启动系统 shell（用户自行运行 `claude` 或其他命令），window 元数据持久化 `worktree_id` 与 `cwd`，重新打开 cc-partner 后应恢复之前打开的 window 列表并保持其所属 worktree
- 终端上下文恢复：macOS/Linux 环境优先使用原生 `tmux` 承载真实 shell 上下文，Windows 环境优先通过默认 WSL 发行版内的 `tmux` 承载上下文（盘符项目路径转换为 `/mnt/<drive>/...`，`\\wsl$\<distro>\...` / `\\wsl.localhost\<distro>\...` 转为发行版内 Linux 路径）；应用退出时只断开 attach，重启后重新 attach 到项目 tmux session 的原 window；没有可恢复 window 时重建 window 并保留 tab 元数据
- 终端输出：PTY 输出必须保留中文与符号等 UTF-8 文本完整性；前端 xterm 必须按 PTY/tmux 原始控制序列渲染，不能启用会改写换行的 `convertEol`；后端启动 Workbench PTY 客户端时必须显式设置 `TERM=xterm-256color` 和真彩色环境，不能继承 `TERM=dumb`；切换 app tab 时必须保留所有 terminal window 的 xterm 实例，只隐藏非活动 window，不能卸载后 replay 原始终端流，同时必须同步底层 tmux current window；前端应轻量同步 tmux current window 到顶部 app tab，不能因为保留 tmux status bar 而让两层 window 选中态分裂；Workbench 路由切出时终端输出缓存必须继续由 AppShell 生命周期的常驻 Provider 接收，切回后 xterm 从该缓存 replay，不能把 terminal output 监听和 buffer 只放在 Workbench 页面内部导致 Claude Code/tmux TUI 丢失屏幕态；buffer 截断导致必须 replay 历史输出时，不得把 xterm 生成的设备能力响应写回 PTY；创建终端时应使用当前终端可见 viewport 的真实 cols/rows 作为 PTY 初始尺寸，终端运行时 resize 也必须按同一可见 viewport 计算，避免交互式程序状态栏和命令行内容出现替换字符、首屏错位或内容超出工作台可视宽度
...
- 会话操作：支持聚焦、读取当前聚焦、重命名、关闭；聚焦 window 必须同步切换底层 tmux current window，读取当前聚焦必须能把 tmux current window id 映射回 app sessionId；关闭/应用退出清理时如果底层进程已自然退出或被系统回收，应视为清理成功，不向用户展示 No such process 类 IO 错误；关闭 tab 会删除持久化 window 并销毁对应 tmux window，应用退出只清理当前 PTY attach，不能删除可恢复 window 元数据或销毁项目 tmux session
...
- 项目文件夹：右侧文件树绑定 active worktree 根目录，支持刷新、展开/收起、选中文件/文件夹、新建文件、新建文件夹、重命名、删除确认、复制相对路径，并展示名称、类型、相对路径、大小、修改时间和父目录
- 第一期不做文件内容预览、图片预览、代码高亮、Markdown 渲染、Git diff 面板、交互式冲突解决和会话日志持久回放
