...
- **Agent Metadata Ledger（A9）**：Agent runtime 终态自动写入 metadata-only 历史（`agent_session_id` 唯一；可靠 usage 可 null-fill；失败不阻断 runtime）。终态时按 provider 从 CLI 本地会话文件提取可靠 usage：Claude（`~/.claude/projects` session JSONL，按 message.id 去重求和）、Codex（`~/.codex/sessions` rollout 最后一个 token_count 累计值）、OpenCode（opencode.db message 表求和）；失败/未提取到保持 null。默认保留 30 天且每设备最多 10,000 条，启动/每 24h 批删 ≤500。本机 drawer 分页（默认 50/最大 200）可滤 project/provider/outcome/time；token 展示分三项：**缓存输入（cache read）/新输入（input）/输出（output）**，unknown token/cost 显示「未提供」（不显示 0）。P2P 仅 `POST /api/workbench/agent-ledger/summary`（capability `workbench.agent-ledger-summary.v1`）暴露 24h/7d/30d aggregate（含 cache read 合计），**无** entry 列表/session id/prompt/path。Fleet 详情 join 7d Agent activity（field 失败不阻断其它摘要；旧 peer unsupported 不显示 0）。Settings 一键清除只删 ledger；不进入 Prompt/SSH/Scratchpad/GitHub sync。
