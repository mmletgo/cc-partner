...
- 终端窗口与 pane：每个工作台项目优先对应一个真实 tmux session，前端 tab 对应 tmux window，window 内分屏对应 tmux pane；没有 tmux、WSL 路径不可转换或后端不可用时退回单普通 PTY window。新建 window 在项目根目录启动系统 shell（用户自行运行 `claude` 或其他命令），支持 window tab 元数据持久化，重新打开 cc-partner 后应恢复之前打开的 window 列表
...
- 终端分屏：中央终端区每次 attach 当前 window；左右/上下分屏调用 tmux `split-window` 创建真实 pane，关闭 pane 调用 tmux `kill-pane`，最后一个 pane 必须通过关闭 window 处理
- 会话状态：右侧只展示当前 window 状态，包含设备、项目名、项目路径、window 名、命令、状态、运行时长、尺寸、开始时间和退出码
- 会话操作：支持重命名、关闭；关闭/应用退出清理时如果底层进程已自然退出或被系统回收，应视为清理成功，不向用户展示 No such process 类 IO 错误；关闭 tab 会删除持久化 window 并销毁对应 tmux window，应用退出只清理当前 PTY attach，不能删除可恢复 window 元数据或销毁项目 tmux session
