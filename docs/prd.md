...
- 依赖环境页签展示 Workbench tmux dependency manager 状态，支持检测、查看后端/版本/路径、查看安装命令预览、触发安装、取消安装和重新检测；同时展示局域网互联访问项目所需的实际 P2P HTTP TCP 端口、mDNS UDP 5353、局域网 IP、当前系统平台、端口是否已开放和对应打开方法。应用只读检测系统防火墙状态，不自动修改防火墙；无法读取到明确放行规则时按未开放显示；移动端 Workbench 的局域网访问链接与二维码由全局侧栏左下角手机按钮弹层展示
