...
| GET | `/api/workbench/events` | `routes/workbench.rs` | none; NDJSON event stream with typed heartbeat every 15s (`{"type":"heartbeat","sentAt":"<RFC3339>"}`) and optional `agentRuntime` frames (capability `workbench.agent-runtime.v1`); unknown `type` must be ignored without reconnect | read-only | client uses lifecycle + per-connection abort; 35s watchdog aborts only the child connection and reconnects while lifecycle remains active; business frames and heartbeats both reset the watchdog |
