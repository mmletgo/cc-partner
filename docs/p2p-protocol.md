...
| POST | `/api/backend/control/agent/query` | `backend/control_agent.rs` | none; typed Agent CLI closed-set queries (project/session/agent/task/experiment/attention/fleet/browser) over owner helpers | read-only | loopback + control-file token; body `{controlToken,op}` camelCase ≤256 KiB; response `{ownerInstanceId,data}` ≤1 MiB; session read must not spawn/restore terminal; never logs token or stdin bodies |
| POST | `/api/backend/control/agent/mutate` | `backend/control_agent.rs` | typed Agent CLI closed-set mutations (worktree create, session send, task create/cancel/retry, experiment create/cancel, browser verify) | no-transport-retry | loopback + control-file token; body `{controlToken,op}` ≤256 KiB; response `{ownerInstanceId,data}` ≤1 MiB; clients classify NeverReplay vs reconcile-by-request-id; never transport-retry NeverReplay; never logs token or body text |
