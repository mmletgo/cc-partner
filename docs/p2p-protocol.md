...
| POST | `/api/orchestrator/task-views/create-block` | `routes/orchestrator.rs` | atomic task block + 2–8 members + create fingerprint | requires-idempotency-key | capability `orchestrator.task-blocks.v1`; body camelCase `{projectId,title,members,createAction,clientRequestId}`; ledger `task_id` stores `block_id`; same key+fingerprint replays members; local project only |
| POST | `/api/orchestrator/task-views/append-block-member` | `routes/orchestrator.rs` | appends one member at max(block_index)+1 | requires-idempotency-key | capability `orchestrator.task-blocks.v1`; allow when head is backlog\|todo\|inProgress; reject humanReview/rework/merging/delivering; max 8 |
| POST | `/api/orchestrator/task-views/reorder-block-members` | `routes/orchestrator.rs` | rewrites block_index by orderedTaskIds permutation | requires-idempotency-key | capability `orchestrator.task-blocks.v1`; all members must be backlog or todo and idle |
...
| POST | `/api/orchestrator/tasks/move-workflow-state` | `routes/orchestrator.rs` | adjacent-lane workflow state move on owner | no-transport-retry | capability `orchestrator.move-workflow-state.v1`; body camelCase `{projectId,taskId,targetState}`; local project only; pending outbox rejected |
