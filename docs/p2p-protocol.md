...
| POST | `/api/workbench/agent-runtime/snapshot` | `routes/workbench.rs` | none; bounded active Agent sessions snapshot (≤1000, sorted by lastActivityAt DESC,id); omits nativeSessionId | read-only | capability `workbench.agent-runtime.v1`; optional body `{projectId?}`; no LAN Hook ingestion write route |
