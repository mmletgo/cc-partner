...
| GET | `/api/mobile/transfer/download/:taskId` | `routes/mobile_transfer.rs` | none; streams completed Receive file | read-only | `application/octet-stream` + `Content-Disposition`; **Receive+completed only**; any other direction/status/missing file → 404 envelope **without** path; never Open/Reveal |
