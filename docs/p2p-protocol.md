...
- `workbench.terminal-paste-image.v1` — paste an image into an Agent TUI (`POST /api/workbench/sessions/paste-image`); owner writes the OS clipboard then injects Ctrl+V (`\x16`); body camelCase `{sessionId,dataUrl}` with `data:image/*;base64,...` ≤8 MiB decoded; missing capability is unsupported; never send image bytes on the 32 KiB terminal-input WebSocket
...
| POST | `/api/workbench/sessions/paste-image` | `routes/workbench.rs` | writes image to owner OS clipboard then PTY Ctrl+V | no-transport-retry | capability `workbench.terminal-paste-image.v1`; camelCase `{sessionId,dataUrl}`; decoded image ≤8 MiB; replay would re-attach the same image |
...
| POST | `/api/mobile/workbench/sessions/paste-image` | `routes/workbench.rs` | writes image to owning-device clipboard then PTY Ctrl+V | no-transport-retry | capability `workbench.terminal-paste-image.v1` on the owning peer; camelCase `{sessionId,dataUrl}`; may two-hop to remote shortcut; never uses the input WebSocket |
