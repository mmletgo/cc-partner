...
- `agent-hub.portable-pull.v1` — same-agent portable asset inventory + selective Pull (`POST /api/agent-hub/portable/inventory`, `POST /api/agent-hub/portable/selection`, `GET /api/agent-hub/portable/objects/:transferId/:objectHash`); token and the three routes ship atomically (dictionary order: `agent-hub.portable-pull.v1` before `agent-hub.v1`)
