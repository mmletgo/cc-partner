...
- `orchestrator.runtime-snapshot.v1` · `orchestrator.agent-adapters.v1` · `orchestrator.complete-agent-run.v1` · `orchestrator.experiments.v1` · `orchestrator.move-workflow-state.v1` — owning-device runtime snapshot / adapters / complete-agent-run (~360s, no-transport-retry) / automated candidate experiments / adjacent-lane board drag. Controller outbox remains a send queue, not the task source of truth.
...
- `workbench.dependency-install.v1` — owning-device tmux status/install/cancel (`POST /api/workbench/dependency/{status,install,cancel}`); headless can spawn_install; sudo without TTY fails closed; missing capability is unsupported
...
| POST | `/api/workbench/dependency/status` | `routes/workbench.rs` | none; reads owner-local tmux dependency runtime | read-only | capability `workbench.dependency-install.v1`; empty body; missing capability is unsupported, not ready |
