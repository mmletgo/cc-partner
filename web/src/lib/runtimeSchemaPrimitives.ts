/**
 * 运行时 DTO decoder 原语实现。
 *
 * Business Logic（为什么需要这个模块）:
 *   runtimeSchema 公共门面需要保持精简；具体组合式 decoder 实现下沉到本文件，
 *   便于维护且不改变 fail-closed 契约与对外 import 路径。
 *
 * Code Logic（这个模块做什么）:
 *   实现 Decoder 接口、ContractDecodeError、actualKindOf 与全部组合式原语；
 *   由 runtimeSchema.ts 再导出，保证既有 `@/lib/runtimeSchema` 消费者零改动。
 */
