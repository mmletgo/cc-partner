 ...
 *
 * Code Logic（这个函数做什么）:
 *   透传 tauriInvoke；catch 后 normalizeError 再抛。
 *
...
/**
 * Business Logic（为什么需要这个函数）:
 *   关键 IPC DTO 必须在写入页面状态前 runtime decode，拒绝损坏/混合版本结构。
 *
 * Code Logic（这个函数做什么）:
 *   先走 normalizeError 的 invoke；成功 body 用 decoder.decode；
 *   ContractDecodeError 原样抛出（不含 payload）；其它错误不吞。
 */
