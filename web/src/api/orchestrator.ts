...
/**
 * Business Logic（为什么需要这个函数）:
 *   Deliver 调用只需 projectId/taskId；A0 后无人工 digest 参数。
 *
 * Code Logic（这个函数做什么）:
 *   projectId/taskId trim 后返回 invoke args。
 */
...
export const orchestratorApi = {
   ...
   *
   * Code Logic（这个函数做什么）:
   *   invokeDecoded deliver_reviewed_orchestrator_task_view。
   */
  deliverReviewedTaskView: (projectId: string, taskId: string) =>
    invokeDecoded(
      ORCHESTRATOR_REMOTE_COMMANDS.deliverReviewedTaskView,
      buildDeliverReviewedOrchestratorTaskViewInvokeArgs(projectId, taskId),
      orchestratorTaskViewDecoder,
    ),
