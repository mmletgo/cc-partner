 ...
 *   HTML 资源重写 helper 的脚本测试需要清晰失败原因，便于在用例失败时快速定位回归。
 *
 * Code Logic（这个函数做什么）:
 *   条件为 false 时抛出 Error，使用例失败。
 */
...
describe('htmlAssets', () => {
   ...
   */
  test('eligibility helper only accepts project-relative asset URLs', () => {
   ...
   */
  test('rewriteCssAssetUrls resolves relative url() against the CSS document path', async () => {
   ...
   */
  test('rewriteHtmlPreviewAssets rewrites link/img/srcset to data URLs and drops externals', async () => {
    const loadAsset = createAssetLoader({
   ...
   */
  test('rewriteHtmlPreviewAssets preserves non-resource links like canonical', async () => {
   ...
   */
  test('rewriteMarkdownPreviewAssets rewrites inline and reference images only', async () => {
    const loadAsset = createAssetLoader({
